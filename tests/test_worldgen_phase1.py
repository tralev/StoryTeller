...
from src.worldgen.artifacts import (
    MAX_GRID_CHUNK_AXIS, MAX_GRID_HEADER_BYTES, ArtifactDependency, ArtifactId,
    ChunkCoordinate, DependencyGraph, FrozenMap, FrozenSequence, GridChunk,
    ProducerFingerprint, WorldArtifact,
    WorldArtifactRepository, artifact_identity_digest, canonical_json,
)
...
from src.worldgen.stages import (
    ...
    StageValidationResult, WorldDiagnostic, WorldStageRunner,
)
...
def test_artifact_identity_dependency_and_producer_contracts() -> None:
    ...
    artifact = WorldArtifact.build(
        "climate", {"temperature": 12_000}, depends_on=(upstream.artifact_id,),
    ...
    with pytest.raises(ValueError, match="duplicate"):
        WorldArtifact.build(
            "climate", {}, depends_on=(upstream.artifact_id, upstream.artifact_id),
...
def test_artifact_identity_cross_platform_vectors_and_domain_separation() -> None:
    fixture = json.loads(Path(
        "tests/fixtures/worldgen/artifact_identity_diagnostics.json"
    ).read_text(encoding="utf-8"))
    assert hashlib.sha256(canonical_json(fixture["payload"])).hexdigest() == fixture["payload_sha256"]
    ids = []
    for vector in fixture["vectors"]:
        digest = artifact_identity_digest(
            vector["kind"], fixture["payload_sha256"], vector["depends_on"],
            vector["producer_fingerprint"],
        )
        built = WorldArtifact.build(
            vector["kind"], fixture["payload"],
    ...
    ordered = WorldArtifact.build(
        "climate", fixture["payload"], depends_on=(left, right),
        producer_fingerprint="producer:v1",
    )
    reversed_dependencies = WorldArtifact.build(
        "climate", fixture["payload"], depends_on=(right, left),
        producer_fingerprint="producer:v1",
    )
...
def test_physical_stage_commit_is_frozen_canonical_input() -> None:
    ...
    stage = PhysicalStageCommit(
        "climate", source, (upstream,), ProducerFingerprint("climate-v1"),
    )
...
def test_wg_kernel_005_contracts_are_immutable_and_typed() -> None:
    immutable_contracts = (
        WorldSpec, StageDependencies, StageInputs, StageOutput, StageRunResult,
        StageValidationResult, WorldDiagnostic, WorldArtifact, PhysicalStageCommit,
        PhysicalWorldResult, WorldCoordinate, LocalCoordinate, ChunkCoordinate, GridChunk,
    )
...
def test_physical_world_result_is_frozen_mapping() -> None:
    ...
    assert canonical_json(result) == (
        b'{"artifacts":14,"maps":5,"regions":7,"routes":6,'
        b'"world_index":"world_index_abc"}'
    )
    with pytest.raises(FrozenInstanceError):
        result.maps = 9
...
def test_grid_chunk_cross_platform_and_malformed_vectors() -> None:
    fixture = json.loads(Path(
        "tests/fixtures/worldgen/grid_chunk_diagnostics.json"
    ).read_text(encoding="utf-8"))
    valid = fixture["valid"]
    chunk = GridChunk(
        valid["layer"], valid["chunk_x"], valid["chunk_y"],
        valid["width"], valid["height"], tuple(valid["values"]),
    )
    ...
    with pytest.raises(ValueError, match="1..256"):
        GridChunk("elevation", 0, 0, MAX_GRID_CHUNK_AXIS + 1, 1,
                  (0,) * (MAX_GRID_CHUNK_AXIS + 1))
    ...
    noncanonical_header = json.dumps({
        "format": "storyteller.grid.i32be.v1", "layer": "elevation",
        "chunk_x": 0, "chunk_y": 0, "width": 1, "height": 1,
    }).encode("utf-8")
    with pytest.raises(ValueError, match="noncanonical"):
        GridChunk.decode(struct.pack(">I", len(noncanonical_header))
                         + noncanonical_header + struct.pack(">i", 0))
...
def test_stage_contracts_are_immutable_canonical_and_typed() -> None:
    ...
    diagnostic = WorldDiagnostic(
        "WG-TERRAIN-COVERAGE", DiagnosticSeverity.WARNING, "coverage is sparse", "cell:7",
    )
