...
def test_prng_decision_contract_and_cross_platform_fixture() -> None:
    fixture = json.loads(
        Path("tests/fixtures/worldgen/prng_diagnostics.json").read_text(encoding="utf-8")
    )
    ...
    for row in fixture["decisions"]:
        seed = derive_seed(
            ...
            row["decision_label"],
            version=fixture["seed_plan_version"],
        )
...
def test_worldgen_division_inventory_is_explicit() -> None:
    ...
    true_divisions: Counter[str] = Counter()
    for path in sorted(Path("src/worldgen").rglob("*.py")):
        tree = ast.parse(path.read_text(encoding="utf-8"))
        for node in ast.walk(tree):
            ...
            if isinstance(node.op, ast.FloorDiv):
                floor_divisions.append(f"{path}:{node.lineno}")
            elif isinstance(node.op, ast.Div):
                true_divisions[str(path)] += 1
    assert floor_divisions == []
    # Every remaining ``/`` is Path composition, except the generated coverage
    # percentage. Per-file counts detect additions without coupling the contract
    # to formatter-induced line movement.
    assert true_divisions == Counter(
        {
            "src/worldgen/artifact_shape_audit.py": 1,
            "src/worldgen/artifacts.py": 2,
            "src/worldgen/biome_reader.py": 2,
            "src/worldgen/climate_reader.py": 2,
            "src/worldgen/conformance/evidence.py": 2,
            ...
            "src/worldgen/grid_catalog_audit.py": 5,
            "src/worldgen/hydrology_reader.py": 2,
            "src/worldgen/index_reader.py": 3,
            "src/worldgen/index_rebuild.py": 2,
            "src/worldgen/local_index.py": 1,
            "src/worldgen/local_reader.py": 15,
            "src/worldgen/maps.py": 4,
            "src/worldgen/physical_pipeline.py": 3,
            "src/worldgen/region_reader.py": 2,
            "src/worldgen/resource_reader.py": 2,
            "src/worldgen/route_reader.py": 1,
            "src/worldgen/simulation/replay.py": 1,
            "src/worldgen/simulation/scheduler.py": 15,
            "src/worldgen/soil_reader.py": 2,
            "src/worldgen/terrain_reader.py": 2,
        }
    )
...
class TestFixedPointContract:
    ...
    @pytest.mark.parametrize(
        ...
        [(5, 2, 3), (4, 2, 2), (1, 2, 1), (-1, 2, -1), (-4, 2, -2), (-5, 2, -3), (0, 7, 0)],
    )
...
def test_stable_id_contract_and_cross_platform_fixture() -> None:
    fixture = json.loads(
        Path("tests/fixtures/worldgen/stable_id_diagnostics.json").read_text(encoding="utf-8")
    )
...
class TestCanonicalJSON:
    ...
    def test_utf16_key_ordering_differs_from_ascii(self) -> None:
        ...
        assert result == '{"a":3,"z":2,"é":1}'.encode()
...
def test_canonical_json_cross_platform_diagnostics_fixture() -> None:
    fixture = json.loads(
        Path("tests/fixtures/worldgen/canonical_json_diagnostics.json").read_text(encoding="utf-8")
    )
    assert fixture["format"] == "storyteller.canonical-json-diagnostics.v1"
    for vector in fixture["vectors"]:
        assert canonical_json(vector["input"]) == vector["expected_utf8"].encode("utf-8"), vector[
            "name"
        ]
