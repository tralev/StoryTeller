from src.domain.artifacts import ArtifactRef
...
def test_resume_verifies_bytes_dependencies_and_producer(tmp_path) -> None:
    ...
    ref = repo.put_bytes("story", "narrative/story.json", b"{}",
                         depends_on=("world_a",), producer_fingerprint="producer")
    store = V2CheckpointStore(tmp_path / "checkpoints.db")
    store.begin_run("run", "fingerprint")
    store.save("run", "phase", "story", "complete", artifact=ref,
               dependencies=("world_a",), producer_fingerprint="producer")
