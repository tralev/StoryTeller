...
def test_precise_geography_government_chronology_and_container_issues(phase4_world):
    ...
    bad_region = replace(
        ...
        neighbors=("region_unknown",),
    )
    bad_civ = replace(
        candidate.civilizations[0], government="impossible", territory=("region_unknown",)
    )
    ...
    bad = replace(
        candidate,
        regions=(bad_region,) + candidate.regions[1:],
        civilizations=(bad_civ,) + candidate.civilizations[1:],
        history=(bad_event,) + candidate.history[1:],
        local_entities=(bad_local,),
    )
...
def test_unknown_identity_route_and_magic_belief_rejected(phase4_world):
    ...
    report = WorldReconciler().reconcile(
        view,
        replace(
            candidate,
            routes=(bad_route,) + candidate.routes[1:],
            magic_claims=(bad_magic,) + candidate.magic_claims[1:],
        ),
    )
...
def test_semantic_critic_cannot_override_deterministic_failure(phase4_world):
    ...
    report = WorldReconciler().reconcile(
        view, candidate, critic_issues=(), critic_status="completed"
    )
    assert not report.accepted
    assert any(issue.code == "WORLD-PRESENT-YEAR" for issue in report.issues)


def test_site_person_temporal_and_projection_contradictions_are_rejected(phase4_world):
    view = WorldView(phase4_world)
    candidate = deterministic_candidate(view, "Ash", "", 1)
    person = replace(candidate.people[0], status="dead", status_year=0)
    event = replace(candidate.history[-1], year=-1, participants=(person.person_id,))
    site = replace(candidate.sites[0], region_id="region_unknown", water_access=False)
    projection = build_projections(view, history_limit=0)
    report = WorldReconciler().reconcile(
        view,
        replace(
            candidate,
            sites=(site,) + candidate.sites[1:],
            people=(person,) + candidate.people[1:],
            history=candidate.history[:-1] + (event,),
        ),
