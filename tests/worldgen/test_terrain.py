...
from src.worldgen.physical_terrain import (_spaced_centers, classify_plate_boundary,
                                           generate_physical_terrain,
                                           LAND_FRACTION_TOLERANCE_PPM,
                                           MAX_ELEVATION_MM, MIN_ELEVATION_MM)
...
def test_exact_continent_count_and_mass_conserving_erosion():
    ...
    assert all(entry.thermal_moved_mm <= grid.cell_count * 16
    ...
    assert all(left.mass_after_mm == right.mass_before_mm
...
def test_textured_continents_are_exact_connected_and_keep_border_ocean(continent_count):
    ...
    terrain = generate_physical_terrain(
        grid, 73, continent_count=continent_count,
        plate_count=max(5, continent_count), erosion_passes=1,
        minimum_continent_cells=4,
    )
    assert set(terrain.continent_id.values) == set(range(continent_count + 1))
    assert all(terrain.continent_id.values[grid.index(x, y)] == 0
    ...
    for number in range(1, continent_count + 1):
        cells = {index for index, value in enumerate(terrain.continent_id.values)
                 if value == number}
        reached = {min(cells)}
        frontier = [min(cells)]
        while frontier:
...
def test_plate_centres_voronoi_motion_and_boundary_contract():
    ...
    terrain = generate_physical_terrain(
        grid, 91, continent_count=1, plate_count=7, erosion_passes=1,
    )
    for index in grid.indices():
        point = grid.coordinate(index)
        expected = min(range(len(centers)), key=lambda plate_index: (
    ...
    assert all(-1_000_000 <= component <= 1_000_000
    ...
    assert classify_plate_boundary(
        grid, Plate("left", left_center, 500_000, 0),
    ...
    assert classify_plate_boundary(
        grid, Plate("left", left_center, 0, -500_000),
...
def test_pathological_four_by_four_grid():
    ...
    terrain = generate_physical_terrain(
        grid, 1, continent_count=1, plate_count=2, erosion_passes=1,
...
def test_requested_land_fraction_is_satisfied(sea_level_ppm):
    ...
    terrain = generate_physical_terrain(
        grid, 83, continent_count=2, plate_count=6, erosion_passes=2,
        minimum_continent_cells=4, sea_level_ppm=sea_level_ppm,
    )
...
def test_unrepresentable_land_fraction_is_rejected_on_pathological_grid():
    with pytest.raises(ValueError, match="WG-LAND-FRACTION"):
        generate_physical_terrain(
            GridSpec(4, 4, 8_000), 1, continent_count=1, plate_count=2,
            erosion_passes=0, sea_level_ppm=50_000,
        )
