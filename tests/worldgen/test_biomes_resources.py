...
def test_biome_resource_and_ecology_artifact_golden_vectors(physical_world):
    ...
    actual = tuple(
        ...
        for layer in (biomes, resources, ecology)
    )
    assert actual == (
...
def test_regional_population_dynamics_are_bounded_and_conservative(physical_world):
    ...
    expected_keys = {(species.species_id, region.region_id)
                     for species in ecology.species for region in regions.regions}
    assert {(item.species_id, item.region_id)
            for item in ecology.regional_populations} == expected_keys
    assert all(item.population == 0 and item.extinct
               for item in ecology.regional_populations if item.carrying_capacity == 0)
    assert all(0 <= item.habitat_suitability_ppm <= 1_000_000
               and item.population >= 0 for item in ecology.regional_populations)
    ...
    assert histories == {(item.species_id, item.region_id): item.population
...
def test_biome_table_is_total_ordered_and_no_mutation(physical_world):
    ...
    assert classify_biome_cell(
        land=1, glacier=0, elevation_mm=100, temperature_millic=10_000,
        precipitation_mm=4_000, drainage_ppm=500_000,
    ) == 6
...
def test_soil_fertility_within_range(physical_world):
    ...
    assert all(value == 0 if not terrain.land.values[i] else 100 <= value <= 5_000
               for i, value in enumerate(soil.depth_mm.values))
    assert all(value in ((0,) if not terrain.land.values[i] else (1, 2, 3))
...
def test_deposits_are_geology_compatible(physical_world):
    ...
    for deposit in resources.deposits:
        ...
        assert deposit.volcanic_related == any(resources.volcano.values[cell] for cell in deposit.cells)
        expected_quantity = div_round_half_up(
            len(deposit.cells) * terrain.grid.metres_per_world_cell ** 2
...
def test_end_to_end_physical_world_publishes_all_artifacts(tmp_path):
    ...
    spec = WorldSpec(width=32, height=32, continent_count=1, plate_count=3,
                     minimum_continent_cells=1, erosion_passes=1,
                     climate_relaxation_passes=8,
                     history_years=0, civilization_count=1)
