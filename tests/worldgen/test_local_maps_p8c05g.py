...
def test_generated_local_maps_have_canonical_golden_vector(phase4_world) -> None:
    ...
    assert hashlib.sha256(canonical_json(maps)).hexdigest() == (
        "7fc3e0313862945d41680fe84b90213b2399276ca20e35d6e5736e954ff31ada"
    )
...
class TestLocalSiteMap:
    ...
    def _make_valid(self, site_id: str = "site_1", seed: int = 0) -> LocalSiteMap:
        ...
        features = (
            ...
            LocalFeature(f"{site_id}_deposit", "mineral_deposit", (cave[0], cave[-1]), ("src_resources",)),
            ...
            LocalFeature(f"{site_id}_parcel", "parcel", tuple(
                (center[0] + dx, center[1] + dy, center[2]) for dx in range(-1, 2) for dy in range(-1, 2)
            ), (site_id,)),
            LocalFeature(f"{site_id}_scar", "event_scar", ((center[0] - 3, center[1], center[2]),), ("src_history",)),
        )
    ...
    def test_rejects_incomplete_geometry(self) -> None:
        ...
        m = self._make_valid()
        bad = LocalSiteMap(1, m.site_id, m.width, m.height, m.z_levels, m.macro_cell,
                           m.strata, (0,), m.features)  # wrong surface
        try:
    ...
    def test_rejects_feature_out_of_bounds(self) -> None:
        ...
        bad = LocalSiteMap(1, m.site_id, m.width, m.height, m.z_levels, m.macro_cell,
                           m.strata, m.surface_height, bad_features)
        try:
            validate_local_map(bad)
            assert False, "should have raised"
        except ValueError as e:
    ...
    def test_rejects_missing_required_features(self) -> None:
        ...
        bad = LocalSiteMap(1, m.site_id, m.width, m.height, m.z_levels, m.macro_cell,
                           m.strata, m.surface_height, reduced)
        try:
            validate_local_map(bad)
            assert False, "should have raised"
        except ValueError as e:
    ...
    def test_rejects_magma_water_overlap(self) -> None:
        ...
        bad = LocalSiteMap(1, m.site_id, m.width, m.height, m.z_levels, m.macro_cell,
                           m.strata, m.surface_height, tuple(overlapped))
        try:
            validate_local_map(bad)
            assert False, "should have raised"
        except ValueError as e:
