...
from src.worldgen.physical_regions import (
    MAX_REGION_CELLS,
    MIN_REGION_CELLS,
    REGION_COST_MODEL,
    TARGET_REGION_CELLS,
...
def test_region_and_route_artifact_golden_vectors(physical_world):
    *_, regions, routes = physical_world
    actual = tuple(hashlib.sha256(canonical_json(layer)).hexdigest() for layer in (regions, routes))
    assert actual == (
...
def test_region_ownership_adjacency_and_routes(physical_world):
    ...
    assert all(
        region.region_id in neighbors[other]
...
def test_multisource_dijkstra_regions_use_all_physical_cost_fields(physical_world):
    ...
    assert len(regions.regions) >= max(
        1, (land_count + TARGET_REGION_CELLS - 1) // TARGET_REGION_CELLS
    )
    assert set(REGION_COST_MODEL) == {
        "base",
        "biome_transition",
        "watershed_transition",
        "elevation_divisor_mm",
        "temperature_divisor_millic",
        "precipitation_divisor_mm",
    }
    costs = [
        region_step_cost(index, neighbor, terrain, hydrology, climate, biomes)
        for index in terrain.grid.indices()
        if terrain.land.values[index]
        for neighbor in terrain.grid.neighbors4(index)
        if terrain.land.values[neighbor]
    ]
...
def test_region_validator_rejects_duplicate_ownership_and_noncanonical_center(physical_world):
    ...
    corrupted = replace(
        regions, regions=(replace(first, center=wrong_center), *regions.regions[1:])
    )
...
def test_route_endpoints_in_declared_regions(physical_world):
    ...
    for route in routes.routes:
        ...
        assert start_region_num is not None, (
            f"route {route.route_id} start cell {start_cell} not in any region"
        )
        assert end_region_num is not None, (
            f"route {route.route_id} end cell {end_cell} not in any region"
        )
...
def test_typed_routes_have_four_valid_seasonal_ast_paths(physical_world):
    ...
    assert {kind for kind in RouteKind} == {
        RouteKind.ROAD,
        RouteKind.TRAIL,
        RouteKind.NAVIGABLE_RIVER,
        RouteKind.SEA_LANE,
        RouteKind.MOUNTAIN_PASS,
        RouteKind.SETTLEMENT_LINK,
    }
    ...
    for route in routes.routes:
        ...
        for season, path in enumerate(route.seasonal_cells):
            ...
            assert all(
                target in terrain.grid.neighbors4(source) for source, target in zip(path, path[1:])
            )
            assert route.traversable_seasons[season] == (
                route.seasonal_capacity[season] > 0 and route.seasonal_risk_ppm[season] < 950_000
            )
...
def test_route_class_rules_costs_maintenance_and_sources_are_frozen(physical_world):
    ...
    assert all(
        set(rule)
        == {"surface", "base_cost", "slope_ppm", "river_cost", "capacity", "maintenance_per_km"}
    ...
    for route in routes.routes:
        ...
        assert route.annual_maintenance == div_round_half_up(
            route.distance_m * int(rule["maintenance_per_km"]),
            1_000,
        )
        assert all(
            bool(terrain.land.values[cell]) == (rule["surface"] == "land")
            for path in route.seasonal_cells
            for cell in path
        )
...
def test_route_validator_rejects_disconnected_pair_and_wrong_endpoint_cell(physical_world):
    ...
    unrelated = next(
        ...
        not in next(
            item for item in regions.regions if item.region_id == first.start_region
        ).neighbors
    )
    mutations = (
        replace(
            first,
            end_region=unrelated.region_id,
...
def test_route_does_not_cross_ocean(physical_world):
    ...
    for route in routes.routes:
        for cell in route.cells:
            assert terrain.land.values[cell], f"route {route.route_id} crosses ocean at cell {cell}"
...
def test_spatial_index_region_lookup(physical_world):
    ...
    assert found == first_region.region_id, f"expected {first_region.region_id}, got {found}"
