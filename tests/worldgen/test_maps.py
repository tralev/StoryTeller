...
def _layers(terrain, regions, routes):
    sources = {
        ...
        for name in ("biome", "climate", "hazard", "hydrology", "political", "resource", "soil", "terrain")
    }
    return build_map_layers(terrain.grid, sources, "regions_" + "2" * 32,
                            "routes_" + "3" * 32, regions, routes)
...
def test_corrupt_raster_is_rejected_without_changing_authoritative_layers(tmp_path, physical_world):
    ...
    layers = _layers(terrain, regions, routes)
    values = {
        "biome": biomes.biome_id.values, "climate": climate.annual_temperature_millic.values,
        "hazard": climate.seasons[0].hazard_ppm.values, "hydrology": hydrology.accumulation.values,
        "political": regions.cell_region.values, "resource": resources.renewable_yield.values,
        "soil": tuple(0 for _ in terrain.grid.indices()), "terrain": terrain.elevation_mm.values,
    }
    paths = render_maps(tmp_path / "maps", terrain, biomes, regions, routes, layers, values)
    manifest = build_map_manifest(tmp_path, paths, layers, regions)
    before = layers
...
def test_layer_reader_rejects_unfrozen_style(physical_world):
    ...
    with pytest.raises(ValueError, match="unknown scalar rendering policy"):
        replace(layers, scalar_layers=(replace(layers.scalar_layers[0], resampling="bilinear"),)
