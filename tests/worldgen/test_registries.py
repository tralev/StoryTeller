...
from src.worldgen.registries import (PHYSICAL_REGISTRIES,
                                     validate_and_hash_physical_registries)
...
@pytest.mark.parametrize(("registry", "direct_stage"), (
    ("biomes", "biomes"), ("materials", "resources"),
    ("species", "species"), ("recipes", "world_index"),
))
def test_registry_changes_invalidate_only_the_direct_physical_producer(registry, direct_stage):
    ...
    differences = {stage for stage in stages
...
def test_registry_validator_rejects_duplicate_ids_and_invalid_versions():
    ...
    duplicate["recipes"] = {"version": 1, "entries": (
        {"id": "same"}, {"id": "same"},
    )}
