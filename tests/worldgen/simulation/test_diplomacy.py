...
def test_diplomacy_war_and_peace_are_typed_replayable_transitions(simulated_world):
    ...
    for transition in transitions:
        source = next(item for item in relation_events if item.event_id == transition["event_id"])
        assert source.participants == (
            transition["left_civilization_id"],
            transition["right_civilization_id"],
        )
...
def test_war_is_the_only_relation_transition_allowed_to_spend_material(simulated_world):
    ...
    cost = next(item for item in war.consequences if item.kind is ConsequenceKind.MATERIAL_DELTA)
