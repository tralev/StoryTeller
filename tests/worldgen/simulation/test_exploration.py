...
def test_exploration_is_route_bounded_unowned_and_provenanced(simulated_world):
    ...
    owned = {
        region
        for civilization in repository.load_verified("civilizations").payload
        for region in civilization["territory"]
    }
    assert discoveries
    assert len(
        {(item["civilization_id"], item["destination_region_id"]) for item in discoveries}
    ) == len(discoveries)
    for discovery in discoveries:
        ...
        assert any(
            item["kind"] == "currency_delta" and item["amount"] == -discovery["currency_cost"]
...
def test_exploration_projector_rejects_teleportation(simulated_world):
    ...
    with pytest.raises(ValueError, match="WG-EXPLORATION"):
        project_exploration_discoveries(
            ...
            state.civilizations,
            state.settlements,
            tuple(item["region_id"] for item in regions),
            tuple(routes),
        )
...
def test_later_conquest_does_not_retroactively_invalidate_exploration(simulated_world):
    ...
    state = _state(repository.load_verified("snapshots").payload[-1]["state"])
    actor = next(item for item in state.civilizations if item.civilization_id == discovery.subject)
    conqueror = next(
        item for item in state.civilizations if item.civilization_id != actor.civilization_id
    )
    last = events[-1]
    conquest = replace(
        last,
        ...
        + (
            Consequence(
                ...
                conqueror.civilization_id,
                1,
                value=discovery.target,
            ),
        ),
    )
    ...
    altered_civilizations = tuple(
        ...
        if item.civilization_id == conqueror.civilization_id
        else item
