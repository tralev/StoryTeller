...
def test_religious_patronage_is_event_sourced_and_identity_linked(simulated_world):
    ...
    religion_events = {item["event_id"]: item for item in history if item["kind"] == "religion"}
    identities = repository.load_verified("identities").payload
    religion_ids = {item["religion_id"] for item in identities["religions"]}
    institution_ids = {item["institution_id"] for item in identities["religious_institutions"]}
    ...
    for patronage in patronages:
        ...
        additions = [
            item for item in event["consequences"] if item["kind"] == "religious_patronage_add"
        ]
...
def test_patronage_projector_rejects_mismatched_holy_site(simulated_world):
    ...
    institutions = tuple(
        ReligiousInstitution(**item) for item in identities["religious_institutions"]
    )
    ...
    malformed = replace(
        ...
        consequences=tuple(
            ...
            if item.kind is ConsequenceKind.RELIGIOUS_PATRONAGE_ADD
            else item
...
def test_event_applier_rejects_material_weight_on_patronage(simulated_world):
    ...
    event = HistoryEvent(
        ...
        EventKind.RELIGION,
        (),
        (state.civilizations[0].civilization_id,),
