...
def _cycle(state):
    ...
    collapse = HistoryEvent(
        ...
        (Consequence(ConsequenceKind.ACTIVE_SET, civilization.civilization_id,
                     value="inactive", details=collapse_details),
    ...
    recovery = HistoryEvent(
        ...
        (civilization.civilization_id,), (civilization.capital_site_id,),
        (Consequence(ConsequenceKind.ACTIVE_SET, civilization.civilization_id,
                     value="active", details=recovery_details),
         Consequence(ConsequenceKind.SETTLEMENT_STATUS_SET, settlement.settlement_id,
                     value=SettlementStatus.INHABITED.value, details=recovery_details)),
...
def test_collapse_recovery_cycle_is_typed_causal_and_identity_preserving(simulated_world):
    ...
    transitions = project_polity_lifecycle(
        42, (collapse, recovery), state.civilizations, state.settlements,
        recovered.civilizations, recovered.settlements,
    )
...
def test_recovery_must_cite_the_collapse_it_reverses(simulated_world):
    ...
    with pytest.raises(ValueError, match="WG-POLITY-LIFECYCLE"):
        project_polity_lifecycle(
            42, (collapse, malformed), state.civilizations, state.settlements,
...
def test_short_history_publishes_empty_verified_lifecycle(simulated_world):
    _, historical, _ = simulated_world
    payload = WorldArtifactRepository(historical / "artifacts").load_verified(
        "polity_lifecycle"
    ).payload
    assert payload == ()
