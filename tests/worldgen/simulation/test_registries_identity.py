...
from src.worldgen.simulation.heraldry import (
    MIN_LUMINANCE_DIFFERENCE_PPM, generate_heraldry, validate_heraldry,
)
...
from src.worldgen.simulation.language_evolution import (
    evolve_language, name_skeleton, realize_syllable, validate_name,
)
from src.worldgen.simulation.magic import (
    EpistemicStatus, MagicEffect, generate_supernatural, validate_supernatural,
)
from src.worldgen.simulation.registries import (
    SIMULATION_REGISTRIES, SIMULATION_STAGE_REGISTRIES,
    simulation_stage_fingerprint, validate_and_hash_registries,
)
...
def test_registries_and_identities_are_retained(simulated_world):
    ...
    assert all(design["motif"]["meaning_source_id"]
               for design in identities["heraldry"].values())
    ...
    assert all(effect["law_id"] in law_ids and effect["source_id"]
               for effect in identities["magic_effects"])
    assert all(religion["attributed_to"] and religion["epistemic_status"] == "uncertain"
    ...
    assert identities["supernatural_places"] and identities["cults"] \
        and identities["sacred_relics"]
...
def test_society_registries_are_complete_versioned_unique_and_stable():
    ...
    assert set(hashes) == {
        ...
        "governments", "beliefs", "magic_vocabulary", "language", "species",
    }
    assert all(registry["version"] == 1 and registry["entries"]
               for registry in SIMULATION_REGISTRIES.values())
    assert hashes == validate_and_hash_registries()


@pytest.mark.parametrize(("registry", "direct_producers"), (
    ("governments", {"civilizations", "government_reforms", "history", "history_clock",
                     "registries", "simulation_index", "snapshots", "successions"}),
    ("materials", {"construction_projects", "economy", "history", "history_clock",
                   "legendary_artifacts", "registries", "settlements", "simulation_index",
                   "snapshots"}),
    ("beliefs", {"history", "history_clock", "identities", "registries",
                 "religious_patronage", "religious_schisms", "simulation_index", "snapshots"}),
    ("language", {"history", "history_clock", "identities", "registries",
                  "simulation_index", "snapshots"}),
))
def test_registry_changes_invalidate_only_declared_simulation_producers(registry, direct_producers):
    ...
    differences = {
        stage for stage in SIMULATION_STAGE_REGISTRIES
...
def test_society_registry_validator_rejects_set_version_and_identity_defects():
    missing = deepcopy(SIMULATION_REGISTRIES); missing.pop("beliefs")
    with pytest.raises(ValueError, match="REGISTRY-SET"):
        validate_and_hash_registries(missing)
    unversioned = deepcopy(SIMULATION_REGISTRIES); unversioned["language"]["version"] = 0
...
def test_cultural_identity_is_stable_and_registry_driven():
    ...
    style = next(entry for entry in SIMULATION_REGISTRIES["language"]["entries"]
                 if entry["id"] == "identity_style_v1")
    phonemes = next(entry for entry in SIMULATION_REGISTRIES["language"]["entries"]
    ...
    if first.language.syllable_pattern.startswith("C"):
        assert all(any(morpheme.startswith(onset) for onset in phonemes["onsets"])
                   for morpheme in first.language.morphemes)
    else:
        assert all(any(morpheme.startswith(vowel) for vowel in phonemes["vowels"])
                   for morpheme in first.language.morphemes)
...
def test_environment_changes_expression_but_not_founder_language_identity():
    ...
    assert (dry.language.morphemes, dry.language.script, dry.flag, dry.heraldry,
            dry.culture_traits) != (wet.language.morphemes, wet.language.script, wet.flag,
                                   wet.heraldry, wet.culture_traits)
...
def test_vector_heraldry_is_deterministic_contrasting_and_culturally_cited():
    ...
    assert abs(first.primary.luminance_ppm - first.secondary.luminance_ppm) \
...
def test_heraldry_validator_rejects_low_contrast_uncited_and_invalid_vectors():
    ...
    with pytest.raises(ValueError, match="HERALDRY-CITATION"):
        validate_heraldry(replace(
            design, motif=replace(design.motif, meaning_source_id=""),
        ))
    with pytest.raises(ValueError, match="HERALDRY-VECTOR"):
        validate_heraldry(replace(
            design, motif=replace(design.motif, center_x_ppm=1_000_001),
        ))
    with pytest.raises(ValueError, match="HERALDRY-COLOR"):
        validate_heraldry(replace(
            design, primary=replace(design.primary, hex_rgb="#FFFFFF"),
        ))
...
def test_objective_magic_effects_cite_law_and_registry_source():
    ...
    assert all(religion.attributed_to == "keeper_circle"
               and religion.epistemic_status is EpistemicStatus.UNCERTAIN
               for religion in religions)
    validate_supernatural(laws, sources, effects, religions, institutions, schisms,
                          interpretations)
...
def test_cosmology_is_layered_attributed_cyclical_and_place_bound():
    ...
    bundle = generate_cosmology(
        37, supernatural.laws, supernatural.sources, supernatural.religions,
        ("site-a", "site-b", "site-c"),
    )
    assert tuple(layer.order for layer in bundle.layers) == (0, 1, 2)
    assert all(cycle.period_months > cycle.phase_offset_months >= 0
               for cycle in bundle.cycles)
    assert {entity.entity_kind for entity in bundle.entities} == {
        "deity", "spirit", "demon", "saint", "false_entity",
    }
    assert all(entity.attributed_to and entity.epistemic_status is not EpistemicStatus.TRUE
               for entity in bundle.entities)
    assert all(claim.attributed_to and claim.destination_layer_id
               for claim in bundle.afterlife_claims)
    ...
    validate_cosmology(bundle, supernatural.laws, supernatural.sources,
                       supernatural.religions, ("site-a", "site-b", "site-c"))
...
def test_cosmology_validator_rejects_uncited_claims_and_forged_place_links():
    ...
    bundle = generate_cosmology(
        41, supernatural.laws, supernatural.sources, supernatural.religions,
        ("site-a", "site-b"),
    )
    with pytest.raises(ValueError, match="COSMOLOGY-ATTRIBUTION"):
        validate_cosmology(
            bundle._replace(entities=(replace(bundle.entities[0], attributed_to=""),
                                      *bundle.entities[1:])),
            supernatural.laws, supernatural.sources, supernatural.religions,
            ("site-a", "site-b"),
        )
    with pytest.raises(ValueError, match="COSMOLOGY-PLACE"):
        validate_cosmology(
            bundle._replace(places=(replace(bundle.places[0], source_id="forged"),
                                    *bundle.places[1:])),
            supernatural.laws, supernatural.sources, supernatural.religions,
            ("site-a", "site-b"),
        )
...
def test_magic_validator_rejects_uncited_or_law_inconsistent_effect():
    ...
    effect = effects[0]
    uncited = MagicEffect(effect.effect_id, "missing-law", effect.source_id, effect.effect,
                          effect.paid_cost, effect.side_effect, effect.location_id)
    with pytest.raises(ValueError, match="EFFECT-SOURCE"):
        validate_supernatural(laws, sources, (uncited,), religions, institutions, schisms,
                              interpretations)
    unpaid = MagicEffect(effect.effect_id, effect.law_id, effect.source_id, effect.effect,
                         "nothing", effect.side_effect, effect.location_id)
    with pytest.raises(ValueError, match="EFFECT-LAW"):
        validate_supernatural(laws, sources, (unpaid,), religions, institutions, schisms,
                              interpretations)


def test_magic_sources_institutions_schisms_and_interpretations_are_place_bound():
    (laws, sources, effects, religions, institutions,
     schisms, interpretations) = generate_supernatural(41, ("site-a", "site-b", "site-c"))
    ...
    assert all(interpretation.epistemic_status is EpistemicStatus.METAPHORICAL
...
def test_language_sound_changes_are_historical_stable_and_keep_language_id():
    identity_design = generate_identity(
        91, "founder-language", set(), CulturePressure(6, 2, True, 2, ("timber",)),
    )
...
def test_persisted_language_history_reaches_configured_year(simulated_world):
    _, historical, _ = simulated_world
    identities = WorldArtifactRepository(historical / "artifacts").load_verified(
        "identities"
    ).payload
    stages_by_language = {}
    for stage in identities["language_history"]:
        stages_by_language.setdefault(stage["language_id"], []).append(stage)
    assert set(stages_by_language) == {language["language_id"] for language in identities["languages"]}
    assert all([stage["year"] for stage in stages] == [0, 25, 50]
               for stages in stages_by_language.values())
