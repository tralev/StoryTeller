...
from src.worldgen.simulation.events import (
    Consequence,
    ConsequenceKind,
    EventKind,
    HistoryEvent,
...
from src.worldgen.simulation.legendary_artifacts import (
    ArtifactProvenance,
    LegendaryArtifact,
    validate_legendary_artifacts,
)
from src.worldgen.simulation.replay import _event
from src.worldgen.simulation.scheduler import (
    _crime_currency_loss,
    _crime_occurs,
    _demographic_change,
    _disaster_losses,
    _disaster_occurs,
    _monthly_economy,
    _route_transport_plan,
    _stock_extraction,
)
from src.worldgen.simulation.settlements import validate_settlements
from src.worldgen.simulation.state import EconomyLedgerEntry, ResourceStock, SettlementStatus
...
def test_scheduler_has_no_raw_floor_division_operators():
    ...
    assert not [node for node in ast.walk(tree) if isinstance(node, ast.FloorDiv)]
...
def test_disaster_trigger_and_losses_are_bounded_deterministic_and_hazard_driven():
    ...
    assert _disaster_occurs(5, "civ-a", 1, 1, 900_000) == _disaster_occurs(
        5, "civ-a", 1, 1, 900_000
    )
...
def test_crime_trigger_and_loss_are_bounded_deterministic_and_pressure_driven():
    ...
    assert _crime_occurs(5, "civ-a", 1, 1, 1_000_000, 0) == _crime_occurs(
        5, "civ-a", 1, 1, 1_000_000, 0
    )
    ...
    routes = (
        {
            ...
            "start_region": "b",
            "end_region": "c",
            "traversable_seasons": (True,) * 4,
            "seasonal_capacity": (7,) * 4,
...
def test_population_cohorts_and_goods_remain_conserved_and_nonnegative(simulated_world):
    ...
    for snapshot in snapshots:
        ...
        for cohort in state["cohorts"]:
            cohort_totals[cohort["civilization_id"]] = (
                cohort_totals.get(cohort["civilization_id"], 0) + cohort["population"]
            )
        assert all(
            civilization["population"] == cohort_totals[civilization["civilization_id"]]
            for civilization in state["civilizations"]
        )
        assert all(
            civilization["population"] >= 0
            and civilization["economy"]["grain"] >= 0
            and civilization["economy"]["currency"] >= 0
            for civilization in state["civilizations"]
        )
        assert all(
            0 <= stock["quantity_kg"] <= stock["capacity_kg"] for stock in state["resource_stocks"]
        )
...
def test_ageing_events_transfer_cohorts_without_changing_population(simulated_world):
    ...
    assert all(
        {item["kind"] for item in event["consequences"]} == {"cohort_transfer"} for event in ageing
    )
    snapshots = repository.load_verified("snapshots").payload
    assert all(
        {cohort["age_band"] for cohort in snapshot["state"]["cohorts"]}
        == {"child", "adult", "elder"}
        for snapshot in snapshots
    )
    for snapshot in snapshots:
        ...
        totals = {}
        for cohort in state["cohorts"]:
            totals[cohort["civilization_id"]] = (
                totals.get(cohort["civilization_id"], 0) + cohort["population"]
            )
        assert totals == {
            civilization["civilization_id"]: civilization["population"]
            for civilization in state["civilizations"]
        }
...
def test_ageing_transfer_rejects_overdraw_and_cross_civilization(simulated_world):
    _, historical, _ = simulated_world
    snapshot = (
        WorldArtifactRepository(historical / "artifacts").load_verified("snapshots").payload[0]
    )
    ...
    same_civ_target = next(
        cohort
        for cohort in state.cohorts
        if cohort.civilization_id == source.civilization_id and cohort.age_band == "adult"
    )
    other_target = next(
        cohort for cohort in state.cohorts if cohort.civilization_id != source.civilization_id
    )
    for target, amount in ((same_civ_target, source.population + 1), (other_target, 1)):
        event = HistoryEvent(
            f"ageing-invalid-{amount}-{target.cohort_id}",
            1,
            12,
            1,
            EventKind.AGEING,
...
def test_disasters_cite_climate_and_apply_exact_bounded_damage(simulated_world):
    ...
    repository = WorldArtifactRepository(historical / "artifacts")
    climate_id = (
        WorldArtifactRepository(physical / "artifacts").load_verified("climate").artifact_id
    )
    disasters = [
        event
        for event in repository.load_verified("history").payload
    ...
    for event in disasters:
        ...
        assert all(
            dict(consequence["details"])["source_id"] == climate_id
            for consequence in event["consequences"]
        )
        assert all(
            0 <= int(dict(consequence["details"])["hazard_ppm"]) <= 1_000_000
            for consequence in event["consequences"]
        )
        population_loss = -sum(
            item["amount"] for item in event["consequences"] if item["kind"] == "population_delta"
        )
        civilization_material_loss = -sum(
            item["amount"] for item in event["consequences"] if item["kind"] == "material_delta"
        )
...
def test_crime_events_have_actor_victim_pressure_resolution_and_exact_loss(simulated_world):
    ...
    repository = WorldArtifactRepository(historical / "artifacts")
    crimes = [
        event for event in repository.load_verified("history").payload if event["kind"] == "crime"
    ]
    assert crimes
    for event in crimes:
        assert (
            len(event["participants"]) == 2 and event["participants"][0] != event["participants"][1]
        )
...
def test_trade_and_migration_are_balanced(simulated_world):
    ...
    for event in history:
        if event["kind"] == "trade":
            assert (
                sum(c["amount"] for c in event["consequences"] if c["kind"] == "grain_delta") == 0
            )
            assert (
                sum(c["amount"] for c in event["consequences"] if c["kind"] == "currency_delta")
                == 0
            )
        if event["kind"] == "migration":
            assert (
                sum(c["amount"] for c in event["consequences"] if c["kind"] == "population_delta")
                == 0
            )
...
def test_economy_ledger_covers_capacity_scarcity_tax_maintenance_and_resources(simulated_world):
    ...
    assert trades
    assert all(
        entry["route_ids"] and 0 < entry["amount"] <= entry["transport_capacity"]
        for entry in trades
    )
    typed = tuple(
        EconomyLedgerEntry(
            ...
            entry["amount"],
            entry["material_id"],
            tuple(entry["route_ids"]),
            entry["transport_capacity"],
        )
...
def test_legendary_artifacts_only_follow_successful_commissions_with_full_provenance(
    ...
    assert any(
        consequence["amount"] < 0 and consequence["details"]
        for consequence in event["consequences"]
    )
...
def test_legendary_artifact_validator_rejects_forged_creation_provenance(simulated_world):
    ...
    artifact = LegendaryArtifact(
        ...
        payload["attributed_meaning"],
        payload["meaning_attributed_to"],
        provenance,
    )
    forged = ArtifactProvenance(
        "event_missing",
        provenance.creation_year,
        provenance.creation_month,
        provenance.creation_sequence,
        provenance.source_ids,
    )
    with pytest.raises(ValueError, match="LEGENDARY-PROVENANCE"):
        validate_legendary_artifacts(
            (replace(artifact, provenance=forged),),
            events,
            people,
            state.civilizations,
            state.settlements,
...
def test_settlement_lifecycle_land_use_workshops_and_inventory_are_retained(simulated_world):
    ...
    settlements = repository.load_verified("settlements").payload
    assert all(
        settlement["status"] == "inhabited" and settlement["founded_year"] == 0
        for settlement in settlements
    )
    assert all(
        settlement["land_use"]
        and settlement["buildings"]
        and settlement["workshops"]
        and settlement["inventory"]
        for settlement in settlements
    )
    assert all(
        workshop["recipe_id"] == "food"
        and workshop["input_material"] == "grain"
        and workshop["output_material"] == "food"
        for settlement in settlements
        for workshop in settlement["workshops"]
    )
    assert all(
        stack["quantity"] >= 0 for settlement in settlements for stack in settlement["inventory"]
    )
    history = repository.load_verified("history").payload
    ...
    assert {item["kind"] for item in construction["consequences"]} >= {
        "material_delta",
        "settlement_building_add",
        "settlement_workshop_add",
    }
    monthly = next(event for event in history if event["kind"] == "monthly_demography")
    inventory_changes = [
        item for item in monthly["consequences"] if item["kind"] == "settlement_inventory_delta"
    ]
...
def test_abandonment_and_recovery_preserve_site_and_settlement_identity(simulated_world):
    _, historical, _ = simulated_world
    snapshot = (
        WorldArtifactRepository(historical / "artifacts")
        .load_verified("snapshots")
        .payload[0]["state"]
    )
    from src.worldgen.simulation.replay import _state
    ...
    after = next(
        item for item in after_state.settlements if item.settlement_id == settlement.settlement_id
    )
    ...
    final = next(
        item for item in final_state.settlements if item.settlement_id == settlement.settlement_id
    )
