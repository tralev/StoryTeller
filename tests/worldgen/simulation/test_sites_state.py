...
from src.worldgen.simulation.scheduler import _load_physical, simulate_world
from src.worldgen.simulation.sites import (
    SITE_SCORE_WEIGHTS,
    CivilizationCapacityError,
    found_sites,
    score_site,
...
def test_sites_are_on_land_and_in_regions(simulated_world):
    ...
    assert all(
        {name for name, _ in site["score_components"]} == expected_components for site in sites
    )
    assert all(0 <= value <= 1_000_000 for site in sites for _, value in site["score_components"])


def test_territory_is_nonoverlapping(simulated_world):
    _, historical, _ = simulated_world
    civilizations = (
        WorldArtifactRepository(historical / "artifacts").load_verified("civilizations").payload
    )
...
def test_configured_civilization_count_is_exact(simulated_world):
    physical, historical, _ = simulated_world
    physical_spec = (
        WorldArtifactRepository(physical / "artifacts").load_verified("world_index").payload["spec"]
    )
...
def test_site_selection_has_stable_cell_and_region_tie_breaking(simulated_world):
    ...
    assert tuple(site.suitability_ppm for site in found_sites(42, physical, 4)) == tuple(
        sorted((site.suitability_ppm for site in found_sites(42, physical, 4)), reverse=True)
    )
...
def test_site_lifecycle_rejects_mutation_deletion_forgery_and_dangling_settlement(simulated_world):
    ...
    with pytest.raises(ValueError, match="SITE-IMMUTABLE"):
        validate_site_lifecycle(
            42,
            state.sites,
            (replace(state.sites[0], cell=state.sites[0].cell + 1),) + state.sites[1:],
            state.settlements,
        )
    with pytest.raises(ValueError, match="SITE-IMMUTABLE"):
        validate_site_lifecycle(42, state.sites, state.sites[:-1], state.settlements)
    forged = (
        replace(state.sites[0], site_id="site_00000000000000000000000000000000"),
    ) + state.sites[1:]
...
def test_households_conserve_cohorts_and_relationships_are_typed_and_retained(simulated_world):
    ...
    validate_relationships(households, people, relationships, state.cohorts, state.settlements)
...
def test_relationship_validator_rejects_population_drift_and_dangling_edges(simulated_world):
    ...
    with pytest.raises(ValueError, match="HOUSEHOLD-CONSERVATION"):
        validate_relationships(
            (replace(households[0], member_count=households[0].member_count - 1), *households[1:]),
            people,
            relationships,
            state.cohorts,
            state.settlements,
    ...
    with pytest.raises(ValueError, match="RELATIONSHIP-EDGE"):
        validate_relationships(
            households, people, (*relationships, forged), state.cohorts, state.settlements
        )
