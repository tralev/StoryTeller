...
def test_reforms_are_typed_non_noop_and_pressure_driven(simulated_world):
    ...
    for reform in reforms:
        ...
        assert any(item["kind"] == "government_set"
        ...
        assert any(item["kind"] == "currency_delta"
                   and item["amount"] == -reform["currency_cost"]
                   for item in event["consequences"])
...
def test_reform_projector_rejects_noop_transition(simulated_world):
    ...
    change = next(item for item in reform.consequences
                  if item.kind is ConsequenceKind.GOVERNMENT_SET)
    forged = replace(
        reform,
        consequences=tuple(replace(item, target=change.value)
                           if item.kind is ConsequenceKind.GOVERNMENT_SET else item
    ...
    with pytest.raises(ValueError, match="WG-REFORM"):
        project_government_reforms(
            42, altered, state.civilizations,
