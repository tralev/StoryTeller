...
def test_technology_unlocks_registry_capability_with_provenance(simulated_world):
    ...
    civilizations = {item["civilization_id"]: item for item in
                     repository.load_verified("civilizations").payload}
    assert discoveries
    for discovery in discoveries:
        event = history[discovery["event_id"]]
        assert event["kind"] == "technology"
        assert discovery["technology_id"] in \
            civilizations[discovery["civilization_id"]]["capabilities"]
        assert discovery["workshop_id"] and discovery["settlement_id"]
        assert any(item["kind"] == "capability_add" and
                   item["target"] == discovery["technology_id"]
                   for item in event["consequences"])
        assert any(item["kind"] == "material_delta" and
                   item["amount"] == -discovery["material_cost"]
                   for item in event["consequences"])
...
def test_technology_projector_rejects_forged_prerequisite(simulated_world):
    ...
    forged = replace(
        ...
        consequences=tuple(
            ...
            )) if item.kind is ConsequenceKind.CAPABILITY_ADD else item
