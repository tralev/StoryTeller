...
def test_artifact_history_rejects_forged_predecessor(simulated_world) -> None:
    ...
    created = next(
        item for item in creation.consequences if item.kind is ConsequenceKind.ARTIFACT_CREATE
    )
    transition = replace(
        creation,
        ...
        consequences=(
            replace(
                ...
                details=(
                    ("transition", "gift"),
                    ("prior_owner_id", created.target),
                    ("prior_site_id", created.value),
                    ("prior_status", "intact"),
                    ("prior_event_id", "forged"),
                    ("new_site_id", created.value),
                    ("new_status", "intact"),
                ),
            ),
        ),
...
def test_artifact_history_supports_every_frozen_lifecycle_transition() -> None:
    creation = HistoryEvent(
        ...
        (
            Consequence(
                ConsequenceKind.ARTIFACT_CREATE, "artifact", target="owner-a", value="site-a"
            ),
        ),
        "created",
    )
    succession = HistoryEvent(
        ...
        (
            Consequence(
                ConsequenceKind.OFFICEHOLDER_SET, "civilization", target="owner-b", value="owner-a"
            ),
        ),
        "succeeded",
    )
    transitions = ("gift", "inheritance", "trade", "theft", "loss", "recovery", "destruction")
    ...
    for index, transition in enumerate(transitions, start=1):
        ...
        event = HistoryEvent(
            ...
            causes,
            tuple(item for item in ("artifact", owner, new_owner) if item),
            (new_site,),
            (
                Consequence(
