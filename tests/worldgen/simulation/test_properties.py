...
def test_history_is_deterministic_and_replayable(tmp_path, seed, years):
    ...
    assert (
        WorldArtifactRepository(first / "artifacts").load_verified("history").sha256
    ...
    history_ids = [
        event["event_id"]
        for event in WorldArtifactRepository(first / "artifacts").load_verified("history").payload
    ]
...
def test_history_clock_rejects_missing_reordered_and_forged_ticks():
    ...
    with pytest.raises(ValueError, match="HISTORY-CLOCK"):
        validate_history_clock(
            2, (replace(clock[0], accepted_event_ids=("forged",)), *clock[1:]), ()
        )
