...
def test_retention_inventory_covers_complete_authoritative_artifacts(simulated_world) -> None:
    ...
    assert inventory["snapshots_sha256"] == hashlib.sha256(canonical_json(snapshots)).hexdigest()
    assert inventory["registries_sha256"] == hashlib.sha256(canonical_json(registries)).hexdigest()
...
def test_extinct_and_abandoned_entities_are_retained_and_discard_is_rejected(
    ...
    settlement = next(
        item for item in genesis.settlements if item.civilization_id == civilization.civilization_id
    )
    final = replace(
        genesis,
        civilizations=tuple(
            replace(item, active=False) if item == civilization else item
            for item in genesis.civilizations
        ),
        settlements=tuple(
            replace(item, status=SettlementStatus.ABANDONED, abandoned_year=1)
            if item == settlement
            else item
    ...
    inventory = build_retention_inventory(
        (),
        (make_snapshot(genesis), make_snapshot(final)),
        {},
        identity_ids,
        genesis,
        final,
