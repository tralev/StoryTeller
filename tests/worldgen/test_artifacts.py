...
from src.worldgen.index_reader import VerifiedSpatialIndexReader
...
def test_all_domains_are_independent_verified_artifacts(tmp_path):
    ...
    assert {f"layer_{layer.layer_id}" for layer in layers.scalar_layers} <= set(
        maps.payload["rasters"]
    )
...
def test_terrain_catalog_reconstructs_complete_model_and_links_dependencies(tmp_path):
    ...
    expected = generate_physical_terrain(
        GridSpec(32, 32, spec.metres_per_world_cell),
        42,
        continent_count=1,
        plate_count=4,
        erosion_passes=2,
    ...
    assert all(
        DenseGridRepository(root / "chunks").load(manifest).spec == verified.grid
...
def test_terrain_elevation_chunk_bytes_are_deterministic_and_corruption_is_rejected(tmp_path):
    ...
    catalog_payload = (
        WorldArtifactRepository(first / "artifacts").load_verified("terrain_grid_catalog").payload
    )
...
def test_hydrology_reader_rejects_chunk_corruption_and_dependency_tampering(tmp_path):
    ...
    encoded = bytearray(chunk.read_bytes())
    encoded[-1] ^= 1
...
def test_physical_stage_dag_and_worker_counts_produce_identical_bytes(tmp_path):
    ...
    assert {path.name: path.read_bytes() for path in (one / "artifacts").glob("*.json")} == {
...
def test_validation_report_binds_the_complete_physical_contract(tmp_path):
    ...
    assert (
        invariants["hydrology"]["river_edge_count"]
...
def test_indexes_delete_and_rebuild_to_exact_bytes_without_touching_sources(tmp_path):
    ...
    generate_physical_world(spec, 42, root)
    index_paths = tuple(
        root / "artifacts" / f"{kind}.json" for kind in ("spatial_index", "reference_index")
    )
    expected = tuple(path.read_bytes() for path in index_paths)
    source_paths = tuple(
        sorted(path for path in (root / "artifacts").glob("*.json") if path not in index_paths)
    )
    ...
    assert rebuilt_ids == tuple(
        WorldArtifactRepository(root / "artifacts").load_verified(kind).artifact_id
...
def test_corrupt_index_is_isolated_and_repairable_from_authoritative_domains(tmp_path):
    ...
    assert VerifiedTerrainReader(root).load().terrain.grid == GridSpec(
        32, 32, spec.metres_per_world_cell
    )
...
def test_invariant_evidence_rejects_adversarial_ledger_and_river_mutations(physical_world):
    ...
    bad_erosion = replace(
        ...
        erosion_ledger=(
            ErosionPassLedger(
                first.pass_index,
                first.mass_before_mm,
                first.thermal_moved_mm,
                first.hydraulic_moved_mm,
    ...
    with pytest.raises(ValueError, match="WG-RIVER"):
        measure_physical_invariants(
            ...
            replace(
                hydrology,
                flow_to=type(hydrology.flow_to)(
                    terrain.grid,
                    tuple(bad_flow),
                ),
            ),
...
def test_physical_pipeline_rejects_invalid_worker_counts(tmp_path, worker_count):
    ...
    with pytest.raises(ValueError, match="WG-WORKERS"):
        generate_physical_world(spec, 42, tmp_path / str(worker_count), worker_count=worker_count)
