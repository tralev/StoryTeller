...
class TestImageStepAtomicWrites:
    """Real ImageGeneratorStep.generate_node writes atomically."""

    @pytest.mark.asyncio
    async def test_generate_node_writes_atomic_files(self, tmp_path: Path) -> None:
        from src.models.image_generator_step import ImageGeneratorStep

        class _FakeImageGen:
            async def generate(self, prompt="", negative_prompt="", size=(512, 512),
                               seed=None, **kwargs: Any) -> bytes:
                return b"\x89PNG\x0d\x0a" + (seed or 0).to_bytes(4, "big") * 64

            async def generate_thumbnail(self, image_bytes: bytes,
                                         size=(128, 128), **kwargs: Any) -> bytes:
                return b"\x89PNG\x0d\x0a" + image_bytes[:32]

        step = ImageGeneratorStep(_FakeImageGen(), output_dir=str(tmp_path))
        img_dir = tmp_path / "images"
        thumb_dir = tmp_path / "thumbnails"

        meta = await step.generate_node(
            "node_01",
            {"node_id": "node_01", "image_prompt": "A scene", "present_characters": [],
             "present_location": ""},
            0,
            {"art_style": {}, "character_design": {}, "location_palettes": {}},
            42,
            img_dir,
            thumb_dir,
        )

        assert Path(meta["image_path"]).read_bytes().startswith(b"\x89PNG")
        assert Path(meta["thumb_path"]).read_bytes().startswith(b"\x89PNG")
        # Atomic: no temp leftovers anywhere
        assert list(tmp_path.rglob("*.tmp")) == []


class TestMusicStepAtomicWrites:
    """Real MusicGeneratorStep.generate_node writes atomically."""

    @pytest.mark.asyncio
    async def test_generate_node_writes_atomic_midi(self, tmp_path: Path) -> None:
        from src.models.music_generator_step import MusicGeneratorStep

        class _FakeTextGen:
            async def generate(self, prompt="", temperature=0.7, seed=None,
                               **kwargs: Any) -> str:
                return ("X:1\nM:4/4\nL:1/8\nK:Dm\n"
                        "D2 F2 A2 d2 | c2 A2 F2 D2 | d8 |]\n")

        class _FakeMusicGen:
            def validate_abc(self, abc: str) -> bool:
                return bool(abc and "X:1" in abc and "K:" in abc)

            def abc_to_midi(self, abc: str) -> bytes:
                return b"MThd" + abc.encode()[:100]

        step = MusicGeneratorStep(_FakeTextGen(), _FakeMusicGen())
        midi_dir = tmp_path / "midi"

        meta = await step.generate_node(
            "node_01",
            {"node_id": "node_01", "music_tone": "melancholy", "text": "scene", "mood": "desolate"},
            0,
            42,
            midi_dir,
        )

        assert Path(meta["midi_path"]).read_bytes().startswith(b"MThd")
        assert list(tmp_path.rglob("*.tmp")) == []
