...
#   bash forge/scripts/run_docker.sh --seed 42 --tone dark_fantasy --title "The Ashen Marches"
#
# Prerequisites:
#   1. Docker installed (docker compose available)
#   2. Models downloaded: bash forge/scripts/pull_models.sh --with-images
...
if [ ! -f "$QWEN" ]; then
    ...
    echo "  Download first: bash forge/scripts/pull_models.sh"
    echo ""
...
if [ ! -f "$SDXL" ]; then
    ...
    echo "  Download: bash forge/scripts/pull_models.sh --with-images"
    echo ""
fi
...
echo "To resume: bash forge/scripts/run_docker.sh --resume"
echo ""
