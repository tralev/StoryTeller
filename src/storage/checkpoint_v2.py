    ...
    def begin_run(self, run_id: str, fingerprint: str) -> None:
        with self._connect() as connection:
            existing = connection.execute(
                "SELECT fingerprint FROM runs WHERE run_id=?", (run_id,)
            ).fetchone()
            if existing and existing[0] != fingerprint:
                raise ValueError("CHECKPOINT_RUN_FINGERPRINT_MISMATCH")
            connection.execute(
                "INSERT OR IGNORE INTO runs VALUES(?,?,?)", (run_id, fingerprint, time.time())
            )
    ...
    def load(self, run_id: str, scope: str, work_id: str) -> WorkCheckpoint | None:
        with self._connect() as connection:
            row = connection.execute(
                """SELECT status,artifact_json,dependencies_json,
              producer_fingerprint FROM work WHERE run_id=? AND scope=? AND work_id=?""",
                (run_id, scope, work_id),
            ).fetchone()
            attempts = connection.execute(
                """SELECT number,code,message,retryable FROM attempts
