...
class _Stage(PipelineStep[Any]):
    def __init__(
        self, name: str, output_key: str, output_dir: str, generator: Any = None, **kwargs: Any
    ) -> None:
...
class BibleV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        world = Path(context.outputs["world"]["path"])
        path = self.root / "bible"
        ...
        cleaned = tuple(
            item.strip() for item in interpretations if isinstance(item, str) and item.strip()
        )
        ...
        return StepOutput(
            {"path": str(path / "bible.json"), "root": str(path), "accepted": report.accepted},
            self.name,
        )
...
class ArtDirectionV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        prompt = (
            ...
            + canonical_json(
                {
                    "climate_palettes": deterministic.climate_palettes,
                    "culture_motifs": deterministic.culture_motifs,
                }
            ).decode("utf-8")
        )
        generated = await self.generator.generate(
        ...
        if (
            not isinstance(palettes, dict)
            or set(palettes) != set(deterministic.climate_palettes)
            or not isinstance(motifs, dict)
            or set(motifs) != set(deterministic.culture_motifs)
        ):
            raise ValueError("ART-DIRECTION-SHAPE: model must preserve every key exactly")
        for values in (palettes, motifs):
            if any(
                not isinstance(value, str) or not value.strip() or len(value.encode("utf-8")) > 2048
                for value in values.values()
            ):
                raise ValueError(
                    "ART-DIRECTION-SHAPE: descriptions must be bounded non-empty strings"
                )
        enriched = ArtDirectionV2(
            deterministic.map_artifact_id,
            deterministic.climate_artifact_id,
            deterministic.accepted_bible_refs,
            {key: palettes[key].strip() for key in sorted(palettes)},
            {key: motifs[key].strip() for key in sorted(motifs)},
            deterministic.world_map,
        )
...
class StoryV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        coverage = generate_story_foundation(
            context.outputs["world"]["path"],
            context.outputs["bible"]["path"],
            path,
        ...
        prompt_scenes = [
            {"scene_id": item["scene_id"], "factual_summary": item["summary"]}
        ...
        generated = await self.generator.generate(
            ...
            temperature=context.spec.temperature,
            seed=context.seed,
            max_tokens=max(2048, len(prompt_scenes) * 256),
        )
        if isinstance(generated, str):
            try:
                generated = json.loads(generated)
        ...
        for item in story_data["scenes"]:
            ...
            for field in ("title", "summary"):
                ...
                if (
                    ...
                    raise ValueError(
                        f"STORY-PROSE-SHAPE: {field} must be a bounded non-empty string"
                    )
                item[field] = value.strip()
        story = _story_from_dict(story_data)
        atomic_write_bytes(path / "story.json", canonical_json(story))
        dependency_ids = tuple(
            sorted(WorldView(context.outputs["world"]["path"]).artifact_ids.values())
        )
        for scene in story.scenes:
            atomic_write_bytes(
                path / "checkpoints" / "story" / f"{scene.scene_id}.json",
                canonical_json({"scene": scene, "dependency_ids": dependency_ids}),
            )
        return StepOutput(
            {"path": str(path / "story.json"), "root": str(path), "coverage": coverage}, self.name
        )


class GraphV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        from ..narrative.pipeline import (
            _graph_from_dict,
            _opportunities_from_dict,
        ...
        prompt_nodes = [
            {"node_id": item["node_id"], "factual_summary": item["text"]}
        ...
        generated = await self.generator.generate(
            ...
            temperature=context.spec.temperature,
            seed=context.seed,
            max_tokens=max(2048, len(prompt_nodes) * 256),
        )
        if isinstance(generated, str):
            try:
                generated = json.loads(generated)
        ...
        opportunities = _opportunities_from_dict(
            json.loads((path / "opportunities.json").read_text())
        )
        ...
        for node in graph.nodes:
            atomic_write_bytes(
                path / "checkpoints" / "graph" / f"{node.node_id}.json",
                canonical_json(
                    {"node": node, "dependency_ids": tuple(sorted(world.artifact_ids.values()))}
                ),
            )
...
class LocalMapsV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        return StepOutput(
            {"path": str(project / "local_maps"), "root": str(project), "coverage": coverage},
            self.name,
        )
...
class MediaIntentsV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        if (
            ...
            or set(generated) != {"nodes"}
            or not isinstance(generated["nodes"], dict)
        ):
...
class GmIndexV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        return StepOutput(
            {"path": str(project / "gm_index.json"), "root": str(project), "coverage": coverage},
            self.name,
        )
...
class PackageV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        path = package_project_v2(
            context.outputs["world"]["path"],
            context.outputs["bible"]["root"],
            context.outputs["narrative_project"]["path"],
            destination,
            title=context.title,
            seed=context.seed,
            staged=True,
...
class PublishPackageV2Stage(_Stage):
    async def generate(self, context: Any) -> StepOutput[dict[str, Any]]:
        ...
        if (
            ...
            or current.manifest.get("content_hash") != acceptance.get("content_hash")
            or current.manifest.get("story_id") != acceptance.get("story_id")
        ):
            raise ValueError(
                "PACKAGE_CHANGED_AFTER_ACCEPTANCE: staged candidate is no longer accepted"
            )
