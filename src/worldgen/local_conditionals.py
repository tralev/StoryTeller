...
def plan_local_conditionals(
    ...
    river_directions = tuple(edge.direction for edge in boundary.edges if edge.river_edge_ids)
    route_directions = tuple(edge.direction for edge in boundary.edges if edge.route_ids)
    aligned = {"east", "west"} if street_axis == "east_west" else {"north", "south"}
    bridge_directions = tuple(
        direction
        for direction in river_directions
...
def synthesize_conditional_features(
    boundary: LocalBoundaryConditions,
    plan: LocalConditionalPlan,
    width: int,
    height: int,
    z_levels: int,
    surface: tuple[int, ...],
    center: tuple[int, int, int],
    cave: tuple[tuple[int, int, int], ...],
    building: tuple[tuple[int, int, int], ...],
    edge_anchors: tuple[tuple[int, int, int], ...],
    ...
    for edge, (x, y, _) in zip(boundary.edges, edge_anchors):
        ...
        if edge.direction in plan.route_directions:
            if x != center[0]:
                ...
                route_cells = tuple(
                    (route_x, center[1], center[2]) for route_x in range(center[0], x + step, step)
                )
            else:
                step = 1 if y > center[1] else -1
                route_cells = tuple(
                    (center[0], route_y, center[2]) for route_y in range(center[1], y + step, step)
                )
            specs.append(
                ConditionalFeatureSpec(
                    f"route_connection_{edge.direction}",
                    "route_connection",
                    route_cells,
                    (artifact_ids["routes"], *edge.route_ids),
                )
            )
