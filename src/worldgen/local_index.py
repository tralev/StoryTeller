...
def build_local_world_index(local_maps: Sequence[object]) -> LocalWorldIndex:
    ...
    for local in sorted(local_maps, key=lambda item: str(getattr(item, "site_id"))):
        ...
        entries.append(LocalWorldIndexEntry(
            site_id,
            ...
            tuple(
                str(getattr(chunk, "sha256"))
                for chunk in getattr(local, "occupancy_chunks")
            ),
            tuple(
                str(getattr(chunk, "sha256"))
                for chunk in getattr(local, "construction_chunks")
            ),
        ))
    return LocalWorldIndex(
        "storyteller.local-world-index.v1",
        "all_registered_sites",
        tuple(entry.site_id for entry in entries), tuple(entries),
    )


def validate_local_world_index(
    index: LocalWorldIndex, local_maps: Sequence[object],
    *, expected_site_ids: Sequence[str] | None = None, local_root: Path | None = None,
) -> None:
    expected = build_local_world_index(local_maps) if index.entries else index
    if (index.format != "storyteller.local-world-index.v1"
            or index.selection_policy != "all_registered_sites"):
        raise ValueError("WG-LOCAL-INDEX-FORMAT: unsupported format")
    if (index.sites != tuple(sorted(set(index.sites)))
    ...
    if any(
        ...
        or any(not HASH_RE.fullmatch(value) for value in (
            *entry.material_chunk_hashes, *entry.occupancy_chunk_hashes,
...
def local_world_index_from_mapping(value: Mapping[str, object]) -> LocalWorldIndex:
    ...
    if (value["format"] != "storyteller.local-world-index.v1"
            or not isinstance(raw_sites, Sequence) or isinstance(raw_sites, (str, bytes))
    ...
    for raw in raw_entries:
        ...
        entries.append(LocalWorldIndexEntry(
            str(raw["site_id"]), str(raw["archive_path"]),
            str(raw["local_map_sha256"]), str(raw["boundary_id"]),
            str(raw["summary_id"]), strings(raw["material_chunk_hashes"]),
    ...
    result = LocalWorldIndex(
        str(value["format"]), str(value["selection_policy"]),
        tuple(raw_sites), tuple(entries),
    )
...
def validate_narrative_independent_coverage(
    index: LocalWorldIndex, registered_site_ids: Sequence[str],
