...
def generate_ecology(biomes: BiomeLayer, regions: RegionLayer, seed: int) -> EcologyLayer:
    ...
    for biome in present:
        base_energy = sum(
            biomes.net_productivity_kg_km2.values[i]
            for i, value in enumerate(biomes.biome_id.values)
            if value == biome
        )
            ...
            species.append(
                Species(
                    stable_id(
                        "species",
                        seed,
                        identity("biome_id", biome),
                        identity("trophic_level", level),
                    ),
                    level,
                    (biome,),
                    energy,
    ...
    for item in species:
        ...
        for region in regions.regions:
            suitable = tuple(
                cell for cell in region.cells if biomes.biome_id.values[cell] in habitat
            )
            suitability = div_round_half_up(len(suitable) * 1_000_000, len(region.cells))
            capacity = div_round_half_up(
                sum(biomes.carrying_capacity.values[cell] for cell in suitable),
                divisor,
            )
            ...
            if capacity:
                rng = rng_for_decision(
                    seed, "ecology_population", item.species_id, region.region_id
                )
                populations[key] = div_round_half_up(
                    capacity * (700_000 + rng.below(500_001)), 1_000_000
                )
            else:
                populations[key] = 0
    ...
    for year in range(1, SIMULATED_YEARS + 1):
        ...
        for key, value in sorted(before.items()):
            ...
            births[key] = (
                div_round_half_up((capacity - value) * 120_000, 1_000_000)
                if value < capacity
                else 0
            )
            deaths[key] = (
                div_round_half_up((value - capacity) * 400_000, 1_000_000)
                if value > capacity
                else 0
            )
        ...
        for item in species:
            for source_id in sorted(region_by_id):
                ...
                available = max(
                    0, before[source_key] - capacities[source_key][1] - deaths[source_key]
                )
                if not available:
                    continue
                targets = []
        ...
        for key in sorted(before):
            after = max(
                0, before[key] + births[key] - deaths[key] + immigrants[key] - emigrants[key]
            )
    ...
    regional = tuple(
        RegionalSpeciesPopulation(
            species_id,
            region_id,
            capacities[(species_id, region_id)][0],
            capacities[(species_id, region_id)][1],
            value,
            value == 0,
        )
    ...
    final_species = tuple(
        Species(
            ...
            item.trophic_level,
            item.habitat_biomes,
            item.annual_energy_kj,
            item.species_id not in living,
        )
        for item in species
    )
    return EcologyLayer(
        ALGORITHM_VERSION, final_species, tuple(food_web), corridors, regional, tuple(ledger)
    )
