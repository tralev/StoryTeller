...
def _path(
    ...
    while frontier:
        ...
        for neighbor in grid.neighbors4(current):
            ...
            if (surface == "land" and not terrain.land.values[neighbor]) or (
                surface == "ocean" and terrain.land.values[neighbor]
            ):
                continue
            step = (
                _rule_integer(kind, "base_cost")
                + div_round_half_up(
                    terrain.slope_ppm.values[neighbor] * _rule_integer(kind, "slope_ppm"), 1_000_000
                )
                + (
                    _rule_integer(kind, "river_cost")
                    if hydrology.accumulation.values[neighbor] > 8
                    else 0
                )
...
def generate_routes(
    ...
    for region in regions.regions:
        for neighbor_id in region.neighbors:
            ...
            preliminary_resource = sum(
                resources.renewable_yield.values[cell] for cell in preliminary
            )
            maximum_slope = max(terrain.slope_ppm.values[cell] for cell in preliminary)
            route_kind = (
                RouteKind.NAVIGABLE_RIVER
                if preliminary_crossings * 2 >= len(preliminary)
                else RouteKind.MOUNTAIN_PASS
                if maximum_slope >= 5_000
                else RouteKind.ROAD
                if preliminary_resource
                else RouteKind.TRAIL
            )
            ...
            risks = tuple(
                div_round_half_up(
                    sum(season.hazard_ppm.values[i] for i in cells),
                    len(cells),
                )
            ...
            traversable = tuple(
                capacity > 0 and risk < 950_000 for capacity, risk in zip(capacity4, risk4)
            )
            ...
            maintenance = div_round_half_up(
                distance_m * _rule_integer(route_kind, "maintenance_per_km"),
                1_000,
            )
            routes.append(
                Route(
                    f"route_{len(routes) + 1:05d}",
                    pair[0],
                    pair[1],
                    cells,
                    distance_m,
