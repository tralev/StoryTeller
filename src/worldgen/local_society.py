...
def derive_cultural_layout(
    seed: int, boundary: LocalBoundaryConditions,
) -> CulturalLocalLayout:
    ...
    return CulturalLocalLayout(
        ...
        2 + rng.below(2),
        boundary.civilization_id, boundary.culture, boundary.settlement_status,
...
def generate_persistent_local_entities(
    ...
    return tuple(
        PersistentLocalEntity(
            stable_id(
                "local_entity", seed, identity("site_id", boundary.site_id),
                identity("ordinal", ordinal), identity("kind", kind),
            ),
            kind, building_cells[ordinal % len(building_cells)],
            boundary.civilization_id, boundary.settlement_id, status,
            boundary.source_artifact_ids,
        )
...
def validate_local_society(
    boundary: LocalBoundaryConditions, layout: CulturalLocalLayout,
    entities: tuple[PersistentLocalEntity, ...],
    building_cells: tuple[tuple[int, int, int], ...],
) -> None:
    if (layout.civilization_id != boundary.civilization_id
...
def cultural_layout_from_mapping(value: Mapping[str, object]) -> CulturalLocalLayout:
    expected = {
        "style_id", "street_axis", "wall_material", "roof_form", "parcel_radius",
        "civilization_id", "culture", "settlement_status", "source_ids",
    }
    if set(value) != expected:
        raise ValueError("WG-LOCAL-LAYOUT-READ: field set mismatch")
    texts = tuple(value[name] for name in (
        "style_id", "street_axis", "wall_material", "roof_form", "civilization_id",
        "culture", "settlement_status",
    ))
    ...
    return CulturalLocalLayout(
        str(texts[0]), str(texts[1]), str(texts[2]), str(texts[3]), radius,
        str(texts[4]), str(texts[5]), str(texts[6]), tuple(str(item) for item in sources),
    )


def persistent_entity_from_mapping(value: Mapping[str, object]) -> PersistentLocalEntity:
    expected = {
        "entity_id", "kind", "cell", "civilization_id", "settlement_id", "status",
        "source_ids",
    }
    if set(value) != expected:
        raise ValueError("WG-LOCAL-ENTITY-READ: field set mismatch")
    cell, sources = value["cell"], value["source_ids"]
    names = tuple(value[name] for name in (
        "entity_id", "kind", "civilization_id", "settlement_id", "status",
    ))
    if (any(not isinstance(item, str) for item in names)
            or not isinstance(cell, Sequence) or isinstance(cell, (str, bytes))
            or len(cell) != 3
    ...
    return PersistentLocalEntity(
        str(names[0]), str(names[1]), (int(cell[0]), int(cell[1]), int(cell[2])),
        str(names[2]), str(names[3]), str(names[4]), tuple(str(item) for item in sources),
    )
