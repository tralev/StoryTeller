...
class LocalPathNotFound(ValueError):
    def __init__(
        ...
        super().__init__(
            f"WG-LOCAL-PATH-NOT-FOUND: {start}->{goal}; reachable={reachable_nodes}"
        )


def _legal_step(
    source: tuple[int, int, int], target: tuple[int, int, int], kind: str,
) -> bool:
...
def build_movement_graph(features: Sequence[object]) -> LocalMovementGraph:
    ...
    for feature in features:
        ...
        for source, target in zip(cells, cells[1:]):
            ...
            edges.add(LocalMovementEdge(
                source, target, kind, MOVEMENT_COSTS[kind], source_ids,
            ))
            edges.add(LocalMovementEdge(
                target, source, kind, MOVEMENT_COSTS[kind], source_ids,
            ))
...
def validate_movement_graph(
    features: Sequence[object], graph: LocalMovementGraph,
) -> None:
...
def find_local_path(
    graph: LocalMovementGraph, start: tuple[int, int, int],
    ...
    start_path = (start,)
    queue: list[tuple[
        int, int, tuple[tuple[int, int, int], ...], tuple[int, int, int],
    ...
    while queue:
        ...
        for edge in adjacency.get(node, ()):
            ...
            heapq.heappush(queue, (
                next_cost + _heuristic(edge.target, goal), next_cost,
...
def _route_anchors(
    features: Sequence[object], route_id: str,
    ...
    if not candidates:
        raise ValueError(
            f"WG-LOCAL-HIERARCHY-ANCHOR: route {route_id} has no local anchor"
        )
    return candidates


def _best_anchor_path(
    graph: LocalMovementGraph, endpoint: tuple[int, int, int],
    anchors: tuple[tuple[int, int, int], ...], *, endpoint_first: bool,
) -> LocalPathResult:
    paths: list[LocalPathResult] = []
    for anchor in anchors:
        try:
            paths.append(find_local_path(
                graph, endpoint, anchor
            ) if endpoint_first else find_local_path(graph, anchor, endpoint))
...
def find_hierarchical_path(
    source_graph: LocalMovementGraph, source_features: Sequence[object],
    source_region: str, start: tuple[int, int, int],
    destination_graph: LocalMovementGraph, destination_features: Sequence[object],
    destination_region: str, goal: tuple[int, int, int],
    ...
    source_path = _best_anchor_path(
        source_graph, start, _route_anchors(source_features, route.route_id),
        endpoint_first=True,
    )
    destination_path = _best_anchor_path(
        destination_graph, goal, _route_anchors(destination_features, route.route_id),
        endpoint_first=False,
    )
    return HierarchicalPathResult(
        route.route_id, source_path, macro_cells, route.cost, destination_path,
        source_path.cost + route.cost + destination_path.cost,
    )


def verified_macro_route(
    world: WorldView, route_id: str,
) -> MacroRouteTraversal:
    ...
    return MacroRouteTraversal(
        route.fact_id, str(route.value["start_region"]), str(route.value["end_region"]),
        tuple(int(cell) for cell in route.value["cells"]),
        int(route.value["terrain_cost"]), route.source_ids,
    )


def find_world_hierarchical_path(
    world: WorldView, source_local: LocalSiteMap, start: tuple[int, int, int],
    destination_local: LocalSiteMap, goal: tuple[int, int, int], route_id: str,
) -> HierarchicalPathResult:
    """Route between two reconciled local maps through verified macro authority."""
    if (source_local.boundary is None or destination_local.boundary is None
    ...
    return find_hierarchical_path(
        source_local.movement_graph, source_local.features,
        source_local.boundary.region_id, start,
        destination_local.movement_graph, destination_local.features,
        destination_local.boundary.region_id, goal,
...
def movement_graph_from_mapping(value: Mapping[str, object]) -> LocalMovementGraph:
    ...
    version, raw_nodes, raw_edges = (
        value["algorithm_version"], value["nodes"], value["edges"]
    )
    ...
    for raw in raw_edges:
        if (not isinstance(raw, Mapping)
                or set(raw) != {"source", "target", "kind", "cost", "source_ids"}):
            raise ValueError("WG-LOCAL-NAV-READ: invalid edge shape")
        kind, cost, sources = raw["kind"], raw["cost"], raw["source_ids"]
        if (not isinstance(kind, str) or isinstance(cost, bool) or not isinstance(cost, int)
        ...
        edges.append(LocalMovementEdge(
            coordinate(raw["source"]), coordinate(raw["target"]), kind, cost,
            tuple(str(item) for item in sources),
        ))
    return LocalMovementGraph(
        version, tuple(coordinate(item) for item in raw_nodes), tuple(edges),
    )
