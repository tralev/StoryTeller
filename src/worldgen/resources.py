    ...
    while frontier and len(body) < target_size:
        ...
        for neighbor in grid.neighbors4(current):
            if (
                ...
                or not terrain.land.values[neighbor]
...
def generate_resources(
    ...
    biomes: BiomeLayer,
    seed: int,
    geology: GeologyLayer | None = None,
) -> ResourceLayer:
    grid = terrain.grid
    ...
    for i in grid.indices():
        ...
        resource = (
            "gems"
            if volcanic_related
            else ("copper" if rock[i] % 2 == 0 else "tin")
            if fault_related
            else _resource_for_cell(geology_layer, i)
        )
        ...
        quantity = div_round_half_up(
            len(cells) * grid.metres_per_world_cell**2 * RESOURCE_DENSITY_KG_M2[resource] * grade,
            1_000_000,
        )
        deposits.append(
            Deposit(
                stable_id("deposit", seed, identity("cell", i)),
                resource,
                cells,
                10_000
                + rng_for_decision(
                ...
                quantity,
                rock[i],
                geology_layer.strata_id.values[i],
                fault_related,
                volcanic_related,
            )
        )
    ...
    return ResourceLayer(
        ALGORITHM_VERSION,
        geology_layer.rock_class_id,
        geology_layer.strata_id,
        geology_layer.parent_material_id,
        geology_layer.fault,
        geology_layer.volcano,
        IntGrid(grid, renewable),
        tuple(deposits),
    )
