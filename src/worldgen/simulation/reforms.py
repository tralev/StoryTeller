...
def project_government_reforms(
    ...
    for event in events:
        changes = [item for item in event.consequences
        ...
        reforms.append(GovernmentReform(
            stable_id("government_reform", seed, identity("event_id", event.event_id)),
            change.subject, change.value, change.target,
            details.get("pressure_kind", ""), pressure_ppm, -cost.amount,
    ...
    for reform in reforms:
        ...
        if (civilization is None or reform.prior_government not in valid_governments
