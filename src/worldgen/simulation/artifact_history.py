...
ARTIFACT_TRANSITIONS = (
    "gift", "inheritance", "trade", "theft", "loss", "recovery", "destruction",
)
...
def project_artifact_histories(
    seed: int, events: tuple[HistoryEvent, ...],
        ...
        for index, consequence in enumerate(event.consequences):
            if consequence.kind is ConsequenceKind.ARTIFACT_CREATE:
                ...
                current[consequence.subject] = (
                    consequence.target, consequence.value, "intact", event.event_id,
                )
                entries.append(ArtifactHistoryEntry(
                    stable_id("artifact_history_entry", seed,
                              identity("event_id", event.event_id),
                              identity("consequence_index", index)),
                    consequence.subject, "creation", "", consequence.target, "",
                    consequence.value, "", "intact", "", event.event_id, event.year,
                ))
                ...
                if (event.kind is not EventKind.ARTIFACT_HISTORY or prior is None
                        ...
                        or (transition == "inheritance" and not any(
                            cause in event_by_id
                            and event_by_id[cause].kind is EventKind.SUCCESSION
                        ...
                        or (transition in {"gift", "inheritance", "trade", "theft"}
                            and (not new_owner or new_status != "intact"))):
                    raise ValueError("WG-ARTIFACT-HISTORY: broken lifecycle transition")
                current[consequence.subject] = (
                    new_owner, new_site, new_status, event.event_id,
                )
                entries.append(ArtifactHistoryEntry(
                    stable_id("artifact_history_entry", seed,
                              identity("event_id", event.event_id),
                              identity("consequence_index", index)),
                    consequence.subject, transition, prior[0], new_owner, prior[1], new_site,
                    prior[2], new_status, prior[3], event.event_id, event.year,
                ))
