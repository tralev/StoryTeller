...
def project_polity_lifecycle(
    ...
    for event in events:
        active_changes = [item for item in event.consequences
                          if item.kind is ConsequenceKind.ACTIVE_SET]
        status_changes = [item for item in event.consequences
                          if item.kind is ConsequenceKind.SETTLEMENT_STATUS_SET]
        if not active_changes and not status_changes:
            continue
        if (event.kind not in {EventKind.COLLAPSE, EventKind.RECOVERY}
                or len(active_changes) != 1 or len(status_changes) != 1):
        ...
        expected_details = {
            ...
            "prior_settlement_status": (
                SettlementStatus.INHABITED.value if is_collapse
                else SettlementStatus.ABANDONED.value),
            "new_settlement_status": (
                SettlementStatus.ABANDONED.value if is_collapse
        ...
        projected.append(PolityLifecycleTransition(
            stable_id("polity_lifecycle", seed, identity("event_id", event.event_id)),
            civilization.civilization_id, settlement.settlement_id,
            expected_details["prior_polity_state"], expected_details["new_polity_state"],
