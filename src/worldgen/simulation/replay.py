...
class ReplayDivergence(ValueError):
    ...
    def __init__(self, boundary_year: int, event_id: str, reason: str) -> None:
        ...
        super().__init__(f"WG-REPLAY-DIVERGENCE: from year {boundary_year}, {location}: {reason}")
...
def validate_history_batch_chain(
    ...
    for path in paths:
        ...
        boundaries.append(
            HistoryBatchBoundary(
                artifact.artifact_id,
                path.stem,
                str(payload["previous_prefix"]),
                str(payload["prefix_sha256"]),
                event_ids,
            )
        )
        previous_artifact_id = artifact.artifact_id
    if (
        (history and not boundaries)
        or prefix != expected_final_prefix
...
def _state(value: dict[str, Any]) -> SimulationState:
    civilizations = tuple(
        CivilizationState(
            ...
            item["government"],
            item["language_id"],
            item["capital_site_id"],
            tuple(item["capabilities"]),
            tuple(item["needs"]),
            tuple(item["territory"]),
            item["population"],
            EconomyState(**item["economy"]),
            item["active"],
        )
    ...
    return SimulationState(
        ...
        tuple(
            SettlementState(
                ...
                item["abandoned_year"],
                tuple(item["land_use"]),
                tuple(item["buildings"]),
                tuple(WorkshopState(**workshop) for workshop in item["workshops"]),
...
def validate_simulation_directory(root: str | Path) -> dict[str, int]:
    ...
    clock = tuple(
        HistoryTick(int(item["year"]), int(item["month"]), tuple(item["accepted_event_ids"]))
        for item in clock_raw
    )
    ...
    for snapshot in snapshots:
        ...
        validate_site_lifecycle(
            seed, genesis_state.sites, snapshot_state.sites, snapshot_state.settlements
        )
    ...
    validate_site_lifecycle(
        seed, genesis_state.sites, published_sites, _state(snapshots[-1]["state"]).settlements
    )
    seen: set[str] = set()
