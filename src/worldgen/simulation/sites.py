...
SITE_SCORE_WEIGHTS = (
    ("fresh_water_ppm", 200_000),
    ("food_capacity_ppm", 200_000),
    ("defense_ppm", 100_000),
    ("safety_ppm", 150_000),
    ("routes_ppm", 100_000),
    ("resources_ppm", 100_000),
    ("climate_ppm", 100_000),
    ("neighbours_ppm", 50_000),
)
...
def validate_site_lifecycle(
    seed: int,
    genesis_sites: tuple[SiteState, ...],
    ...
    for site in current_sites:
        expected = stable_id(
            "site",
            seed,
            identity("region_id", site.region_id),
            identity("cell", site.cell),
        )
...
def score_site(physical: dict[str, Any], region: dict[str, Any], cell: int) -> SiteSuitability:
    ...
    water = bool(
        hydrology["coastline"]["values"][cell] or hydrology["accumulation"]["values"][cell] > 4
    )
    resource = bool(
        resources["renewable_yield"]["values"][cell]
        or any(cell in deposit["cells"] for deposit in resources["deposits"])
    )
    route_degree = sum(
        str(region["region_id"]) in (route["start_region"], route["end_region"])
        for route in physical["routes"]["routes"]
    )
    hazards = tuple(int(season["hazard_ppm"]["values"][cell]) for season in climate["seasons"])
    ...
    values = {
        ...
        "climate_ppm": max(
            0, 1_000_000 - div_round_half_up(temperature_penalty + precipitation_penalty, 2)
        ),
        "neighbours_ppm": min(1_000_000, len(region["neighbors"]) * 250_000),
    }
    total = sum(
        div_round_half_up(values[name] * weight, 1_000_000) for name, weight in SITE_SCORE_WEIGHTS
    )
...
def found_sites(seed: int, physical: dict[str, Any], count: int) -> tuple[SiteState, ...]:
    ...
    for region in physical["regions"]["regions"]:
        ...
        for raw_cell in region["cells"]:
            ...
            if breakdown.food_capacity_ppm > 0 and (
                breakdown.fresh_water_ppm > 0 or breakdown.resources_ppm > 0
            ):
    ...
    if len(scores) < count:
        raise CivilizationCapacityError(
            CivilizationCapacityDiagnostic(
                "WG-CIV-CAPACITY",
                count,
                len(scores),
    ...
    return tuple(
        SiteState(
            ...
            score,
            bool(
                physical["hydrology"]["coastline"]["values"][cell]
                or physical["hydrology"]["accumulation"]["values"][cell] > 4
            ),
            bool(
                physical["resources"]["renewable_yield"]["values"][cell]
