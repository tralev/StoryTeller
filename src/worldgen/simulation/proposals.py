...
def conflict_sort_key(proposal: HistoryProposal) -> tuple[object, ...]:
    ...
    return (proposal.year, proposal.month, proposal.priority, proposal.kind.value,
            proposal.actor_id, proposal.proposal_id)
...
def resolve_proposals(
    ...
    for proposal in proposals:
        if (not proposal.proposal_id or not proposal.participants or not proposal.locations
                or not proposal.consequences or proposal.priority < 0
    ...
    for proposal in sorted(proposals, key=conflict_sort_key):
        blockers = tuple(sorted({claimed_by[key] for key in proposal.conflict_keys
        ...
        decisions.append(ProposalDecision(
            proposal.proposal_id, is_accepted, proposal.conflict_keys, blockers,
