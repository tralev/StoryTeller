...
def validate_cosmology(bundle: CosmologyBundle, laws: tuple[MagicLaw, ...],
                       sources: tuple[MagicSource, ...], religions: tuple[Religion, ...],
                       site_ids: tuple[str, ...]) -> None:
    layer_ids = {layer.layer_id for layer in bundle.layers}
    if (not bundle.layers or len(layer_ids) != len(bundle.layers)
    ...
    if any(not attributed_to or not isinstance(status, EpistemicStatus)
    ...
    if any(cycle.period_months <= 0 or not 0 <= cycle.phase_offset_months < cycle.period_months
    ...
    if (len(entity_ids) != len(bundle.entities)
            or any(entity.entity_kind not in allowed_entities or entity.layer_id not in layer_ids
    ...
    if len(place_ids) != len(bundle.places) or any(
            place.site_id not in known_sites or place.phenomenon_kind not in {"hazard", "resource"}
            or place.law_id not in law_ids or place.source_id not in source_by_id
    ...
    if any(cult.religion_id not in religion_ids or cult.entity_id not in entity_ids
           or cult.site_id not in known_sites or not cult.rite for cult in bundle.cults):
        raise ValueError("WG-COSMOLOGY-CULT: invalid cult, rite, or site")
    if any(relic.religion_id not in religion_ids or relic.site_id not in known_sites
           or not relic.attributed_power for relic in bundle.relics):
        raise ValueError("WG-COSMOLOGY-RELIC: invalid attributed relic")


def generate_cosmology(seed: int, laws: tuple[MagicLaw, ...], sources: tuple[MagicSource, ...],
                       religions: tuple[Religion, ...], site_ids: tuple[str, ...]) -> CosmologyBundle:
    ...
    if (not isinstance(layer_names, tuple) or not isinstance(cycles_raw, tuple)
    ...
    layers = tuple(CosmologicalLayer(
        stable_id("cosmological_layer", seed, identity("layer_index", index)), index,
        str(name), f"{name} surrounds the mortal world", attribution, EpistemicStatus.UNCERTAIN,
    ...
    for cycle_raw in cycles_raw:
        ...
        cycles.append(CelestialCycle(
            stable_id("celestial_cycle", seed, identity("cycle_index", len(cycles))), name, period,
            seed % period, f"the {name} marks passages between layers", attribution,
            EpistemicStatus.METAPHORICAL,
        ))
    entities = tuple(AttributedEntity(
        stable_id("cosmological_entity", seed, identity("kind", str(kind))), str(kind),
        f"The {str(kind).replace('_', ' ').title()}", layers[index % len(layers)].layer_id,
        f"a {str(kind).replace('_', ' ')} watches the {layers[index % len(layers)].name}",
        attribution, EpistemicStatus.FALSE if kind == "false_entity" else EpistemicStatus.UNCERTAIN,
    ) for index, kind in enumerate(entity_kinds))
    afterlife = (AfterlifeClaim(
        stable_id("afterlife_claim", seed, identity("layer_id", layers[-1].layer_id)),
        layers[-1].layer_id, f"the remembered dead travel to {layers[-1].name}",
        attribution, EpistemicStatus.UNCERTAIN,
    ),)
    source_by_site = {source.location_id: source for source in sources}
    places = tuple(SupernaturalPlace(
        stable_id("supernatural_place", seed, identity("site_id", site_id)), site_id,
        "hazard" if index % 2 == 0 else "resource",
        "light-draining resonance" if index % 2 == 0 else "light-bearing resonance",
        source.law_id, source.source_id,
    ) for index, site_id in enumerate(site_ids) if (source := source_by_site.get(site_id)))
    cults = tuple(Cult(
        stable_id("cult", seed, identity("religion_id", religion.religion_id)),
        religion.religion_id, entities[index % len(entities)].entity_id, religion.holy_site_id,
        str(raw["rite"]),
    ) for index, religion in enumerate(religions))
    relics = tuple(SacredRelic(
        stable_id("sacred_relic", seed, identity("religion_id", religion.religion_id)),
        religion.religion_id, religion.holy_site_id, f"Waystone of {index + 1}",
        "said to remember every pilgrim", cult.cult_id, EpistemicStatus.UNCERTAIN,
