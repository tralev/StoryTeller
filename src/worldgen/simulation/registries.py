...
SIMULATION_REGISTRIES: dict[str, dict[str, object]] = {
    ...
    "materials": {
        "version": 1,
        "entries": tuple(dict(entry) for entry in registry_entries("materials")),
    },
    "recipes": {
        "version": 1,
        "entries": tuple(dict(entry) for entry in registry_entries("recipes")),
    },
    "institutions": {
        "version": 1,
        "entries": (
    ...
    "beliefs": {
        ...
        "entries": (
            {
                "id": "ancestor_roads",
                "claim": "ancestors guard remembered roads",
                "institution": "keeper_circle",
                "taboo": "destroying a waystone",
                "rite": "walking the remembered circuit",
            },
            {
                "id": "cosmology_v1",
                "layers": ("mortal sphere", "star road", "remembered shore"),
                "cycles": (
                    {"name": "silver return", "period_months": 18},
        ...
        "entries": (
            {
                "id": "resonance",
                "cost": "fatigue",
                "limit": "cannot create energy",
                "prohibited": ("create_matter", "resurrection", "rewrite_history"),
                "source_kind": "place-bound resonance",
                "side_effect": "temporary color blindness",
            },
        ),
    },
    "language": {
        ...
        "entries": (
            ...
            {
                "id": "identity_style_v1",
                "scripts": ("runes", "glyphs", "knots"),
            ...
            },
            {
                ...
                "biome_traits": {
                    "1": "ice-season memory",
                    "2": "highland stonecraft",
                    "3": "cold-steppe endurance",
                    "4": "dryland waterkeeping",
                    "5": "grassland horsemanship",
                    "6": "woodland stewardship",
                    "7": "rainforest canopy craft",
                    "8": "wetland navigation",
                },
                "climate_traits": (
                    "cold-weather planning",
                    "seasonal husbandry",
                    "temperate cultivation",
                    "storm-season architecture",
                    "heat-adapted scheduling",
                ),
                "water_trait": "waterside navigation",
                "route_trait": "crossroads exchange",
    ...
    "species": {
        "version": 1,
        "entries": tuple(dict(entry) for entry in registry_entries("species")),
    },
}
...
def validate_and_hash_registries(
    ...
    for name, registry in sorted(registries.items()):
        ...
        if (
            ...
            or any(item is None for item in ids)
        ):
            raise ValueError(f"WG-REGISTRY-DUPLICATE: {name}")
        for entry in entries:
            ratio = entry.get("ratio_ppm")
            if ratio is not None and (
                isinstance(ratio, bool) or not isinstance(ratio, int) or not 0 < ratio <= 1_000_000
            ):
...
def simulation_stage_fingerprint(
    kind: str, history_years: int, registry_hashes: Mapping[str, str]
) -> str:
