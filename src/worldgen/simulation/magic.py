...
def validate_supernatural(
    ...
    for effect in effects:
        ...
        if (
            effect.effect != law.effect
            or effect.paid_cost != law.cost
            or not effect.side_effect
            or source.law_id != law.law_id
            or source.location_id != effect.location_id
        ):
    ...
    if any(
        schism.parent_religion_id not in religion_ids
        or schism.child_institution_id not in institution_ids
        or not schism.disputed_claim
        for schism in schisms
    ):
        raise ValueError("WG-MAGIC-SCHISM: invalid schism ancestry")
    if any(
        interpretation.religion_id not in religion_ids
        or interpretation.law_id not in law_ids
        or not interpretation.attributed_to
...
def generate_supernatural(
    ...
    law = MagicLaw(
        ...
        "alter perceived light",
        str(vocabulary["cost"]),
        str(vocabulary["limit"]),
    ...
    schisms = tuple(
        Schism(
            stable_id(
                "schism",
                seed,
                identity("parent_religion_id", religions[0].religion_id),
                identity("child_institution_id", institution.institution_id),
            ),
            religions[0].religion_id,
    ...
    interpretations = tuple(
        CulturalInterpretation(
            stable_id(
                ...
                seed,
                identity("religion_id", religion.religion_id),
                identity("law_id", law.law_id),
            ),
            religion.religion_id,
            law.law_id,
    ...
    result = SupernaturalBundle(
        (law,), sources, effects, religions, institutions, schisms, interpretations
    )
