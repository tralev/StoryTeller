...
def _event_state_hash(state: SimulationState, event: HistoryEvent) -> str:
    ...
    payload = {
        ...
        "civilizations": tuple(
            asdict(item) for item in state.civilizations if item.civilization_id in ids
        ),
        ...
        "relations": tuple(
            asdict(item) for item in state.relations if item.left in ids or item.right in ids
        ),
        "resource_stocks": tuple(
            asdict(item) for item in state.resource_stocks if item.stock_id in ids
        ),
    }
...
def ordered_events(events: tuple[HistoryEvent, ...]) -> tuple[HistoryEvent, ...]:
    return tuple(
        sorted(events, key=lambda event: (event.year, event.month, event.sequence, event.event_id))
    )
...
def apply_event(state: SimulationState, event: HistoryEvent) -> SimulationState:
    ...
    if any(cause not in known_events for cause in event.causes):
        raise ValueError(
            f"WG-EVENT-CAUSE: cause must reference an earlier applied event: {event.event_id}"
        )
    civilizations = {
        civilization.civilization_id: civilization for civilization in state.civilizations
    }
    ...
    for operation_index, operation in enumerate(event.consequences):
        ...
        if operation.kind is ConsequenceKind.ARTIFACT_TRANSITION:
            ...
            if (
                operation.amount != 0
                or not operation.subject
                or not details.get("transition")
                or not details.get("prior_event_id")
                or not details.get("prior_status")
                or not details.get("new_status")
                or not details.get("prior_site_id")
                or not details.get("new_site_id")
            ):
        ...
        if operation.kind is ConsequenceKind.PERSON_STATUS_SET:
            ...
            if (
                operation.amount != 0
                or operation.value not in {"living", "dead"}
        ...
        if operation.kind is ConsequenceKind.COHORT_TRANSFER:
            ...
            cohorts[source.cohort_id] = replace(
                source, population=source.population - operation.amount
            )
            cohorts[target.cohort_id] = replace(
                target, population=target.population + operation.amount
            )
            continue
        if operation.kind == ConsequenceKind.ECONOMY_LEDGER_APPEND:
            details = dict(operation.details)
            route_ids = tuple(filter(None, details.get("route_ids", "").split(",")))
            economy_ledger.append(
                EconomyLedgerEntry(
                    f"{event.event_id}:{operation_index}",
                    event.year,
                    event.month,
                    operation.value,
                    operation.subject,
        ...
        if operation.kind == ConsequenceKind.SETTLEMENT_INVENTORY_DELTA:
            ...
            inventory[operation.target] = replace(
                current,
                quantity=max(0, current.quantity + operation.amount),
            )
            settlements[operation.subject] = replace(
                settlement,
                inventory=tuple(inventory[key] for key in sorted(inventory)),
            )
            continue
        if operation.kind == ConsequenceKind.SETTLEMENT_BUILDING_ADD:
            ...
            settlements[operation.subject] = replace(
                settlement,
                buildings=tuple(sorted(settlement.buildings + (operation.value,))),
            )
            continue
        if operation.kind == ConsequenceKind.SETTLEMENT_WORKSHOP_ADD:
            ...
            workshop = WorkshopState(
                parts[0], parts[1], parts[2], parts[3], parts[4], int(parts[5])
            )
            settlements[operation.subject] = replace(
                settlement,
                workshops=tuple(
                    sorted(settlement.workshops + (workshop,), key=lambda item: item.workshop_id)
                ),
            )
            continue
        ...
        if operation.kind == ConsequenceKind.RESOURCE_STOCK_DELTA:
            ...
            stocks[operation.subject] = replace(
                stock,
                quantity_kg=max(0, min(stock.capacity_kg, stock.quantity_kg + operation.amount)),
            )
            continue
        if operation.kind in (
            ...
            if operation.kind == ConsequenceKind.POPULATION_DELTA:
                civilization = replace(
                    civilization, population=max(0, civilization.population + operation.amount)
                )
                age_order = (
                    {"child": 0, "adult": 1, "elder": 2}
                ...
                if matching:
                    ...
                    cohorts[matching[0]] = replace(
                        cohort, population=cohort.population + operation.amount
                    )
                ...
                if settlement_keys:
                    ...
                    settlements[settlement_keys[0]] = replace(
                        settlement,
                        population=max(0, settlement.population + operation.amount),
                    )
            ...
            if (
                operation.subject not in civilizations
                or operation.target not in civilizations
                or operation.subject == operation.target
            ...
            relations[(pair[0], pair[1])] = DiplomaticRelation(
                pair[0], pair[1], operation.value, operation.amount
            )
    result = replace(
        state,
        year=event.year,
