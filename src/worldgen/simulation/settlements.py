...
def validate_settlements(settlements: tuple[SettlementState, ...]) -> None:
    ...
    for settlement in settlements:
        if (
            not settlement.site_id
            or settlement.founded_year < 0
            or settlement.carrying_capacity < 1
        ):
        ...
        if (
            settlement.status is SettlementStatus.INHABITED
            and settlement.abandoned_year is not None
        ):
            raise ValueError(f"WG-SETTLEMENT-ABANDONMENT: {settlement.settlement_id}")
        ...
        if len(workshop_ids) != len(settlement.workshops) or any(
            workshop.recipe_id not in recipe_ids
            or not workshop.input_material
            or not workshop.output_material
            or not 0 < workshop.ratio_ppm <= 1_000_000
        ...
        if len(materials) != len(set(materials)) or any(
            stack.quantity < 0 for stack in settlement.inventory
        ):
