...
def generate_megabeasts(
    ...
    species = sorted(
        (
            item
            for item in physical["ecology"]["species"]
    ...
    for item in species:
        compatible = [
            ...
            and any(
                biome_values[cell] in item["habitat_biomes"]
        ...
        result.append(
            Megabeast(
                stable_id(
                    "megabeast",
                    seed,
                    identity("species_id", item["species_id"]),
                    identity("ordinal", index),
                ),
                item["species_id"],
                ("siege_strength", "territorial_memory"),
                tuple(item["habitat_biomes"]),
                lair.site_id,
                lair.region_id,
                territory,
                1,
        ...
        if len(result) == count:
            break
    if (
        len(result) > max(2, div_floor_exact(len(sites), 2))
...
def project_megabeast_history(
    ...
    for event in events:
        if event.kind not in MEGABEAST_EVENT_KINDS:
            continue
        consequences = [
            item for item in event.consequences if item.kind is ConsequenceKind.MEGABEAST_TRANSITION
        ]
        ...
        if (
            ...
            or (
                is_origin
                and (
                    beast.megabeast_id in originated
                    or prior[2]
