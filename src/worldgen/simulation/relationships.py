...
def _relationship_types() -> tuple[str, ...]:
    entry = next(
        item
        for item in simulation_registry_entries("people")
...
def validate_relationships(
    ...
    if any(totals[cohort.cohort_id] != cohort.population for cohort in cohorts):
        raise ValueError(
            "WG-HOUSEHOLD-CONSERVATION: household members must equal cohort population"
        )
    ...
    if any(
        (person.cohort_id, person.settlement_id, person.civilization_id)
        != (
    ...
    parents = {
        ...
        for relation in relationships
        if relation.relationship_type == "parent_of"
    }
...
def generate_relationships(
    ...
    settlements: tuple[SettlementState, ...],
    present_year: int,
) -> tuple[tuple[Household, ...], tuple[SocialAnchor, ...], tuple[PersonalRelationship, ...]]:
    ...
    for cohort in sorted(cohorts, key=lambda item: item.cohort_id):
        ...
        for index in range(household_count):
            ...
            cohort_households.append(household)
        anchors = []
        ...
        edge_specs.extend(
            (anchors[index], anchors[index + 1], "spouse")
            for index in range(0, len(anchors) - 1, 2)
        )
        edge_specs.extend(
            (anchors[index], anchors[index + 2], "parent_of")
            for index in range(0, len(anchors) - 2, 3)
        )
        edge_specs.extend(
            (anchors[index], anchors[index + 1], "mentor")
