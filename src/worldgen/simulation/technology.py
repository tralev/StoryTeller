...
def project_technology_discoveries(
    ...
    for event in events:
        unlocks = [
            item for item in event.consequences if item.kind is ConsequenceKind.CAPABILITY_ADD
        ]
        if event.kind is not EventKind.TECHNOLOGY:
            if unlocks:
                raise ValueError("WG-TECHNOLOGY-EVENT: capability outside discovery")
            continue
        costs = [item for item in event.consequences if item.kind is ConsequenceKind.MATERIAL_DELTA]
    ...
    for discovery in sorted(
        discoveries,
        key=lambda item: (
            event_by_id[item.event_id].year,
            event_by_id[item.event_id].month,
            event_by_id[item.event_id].sequence,
            item.discovery_id,
        ),
    ):
        ...
        workshop_ids = (
            set() if settlement is None else {item.workshop_id for item in settlement.workshops}
        )
        ...
        if (
            technology is None
            or source_civilization is None
            or settlement is None
            or settlement.civilization_id != discovery.civilization_id
            or discovery.workshop_id not in workshop_ids
            or discovery.prerequisites != required
            or not set(required) <= known
            or discovery.technology_id in known
            or discovery.material_cost <= 0
