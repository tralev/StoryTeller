...
def region_step_cost(
    source: int, target: int, terrain: Terrain, hydrology: Hydrology,
    climate: ClimateLayer, biomes: BiomeLayer,
) -> int:
    return (
        REGION_COST_MODEL["base"]
        + (REGION_COST_MODEL["biome_transition"]
           if biomes.biome_id.values[source] != biomes.biome_id.values[target] else 0)
        + (REGION_COST_MODEL["watershed_transition"]
           if hydrology.watershed_id.values[source] != hydrology.watershed_id.values[target] else 0)
        + div_round_half_up(abs(terrain.elevation_mm.values[source]
                                - terrain.elevation_mm.values[target]),
                            REGION_COST_MODEL["elevation_divisor_mm"])
        + div_round_half_up(abs(climate.annual_temperature_millic.values[source]
...
def _split_oversized(cells: set[int], terrain: Terrain) -> list[set[int]]:
    ...
    left = {cell for cell in cells
            if (first_distance[cell], first) <= (second_distance[cell], second)}
    right = cells - left
...
def _normalize_region_cells(raw: list[set[int]], terrain: Terrain) -> list[set[int]]:
    ...
    while len(regions) > 1:
        small_index = next((index for index, cells in enumerate(regions)
                            if len(cells) < MIN_REGION_CELLS), None)
        ...
        for target_index, target in enumerate(regions):
            if target_index == small_index:
                continue
            contact = sum(neighbor in target for cell in small
                          for neighbor in terrain.grid.neighbors4(cell))
            if contact:
        ...
        regions = [cells for index, cells in enumerate(regions)
...
def generate_regions(
    terrain: Terrain, hydrology: Hydrology, climate: ClimateLayer, biomes: BiomeLayer,
) -> RegionLayer:
    grid = terrain.grid
    ...
    while frontier:
        ...
        for neighbor in grid.neighbors4(index):
            ...
            candidate = cost + region_step_cost(
                index, neighbor, terrain, hydrology, climate, biomes,
            )
    ...
    initial = [set(index for index, value in enumerate(owner) if value == number)
    ...
    for number, region_cells in enumerate(regions_raw, 1):
        ...
        mean_y = div_round_half_up(sum(grid.coordinate(i).y for i in region_cells), len(region_cells))
