...
from .artifacts import (
    ProducerFingerprint, WorldArtifact, WorldArtifactRepository, canonical_json,
    freeze_canonical,
)
from .grid import (
    DenseGridCatalog, DenseGridManifest, DenseGridRepository, GridSpec, IntGrid, build_grid_manifest,
    iter_grid_chunks,
)
...
from .reference_index import ReferenceIndex, reference_index_payload, validate_reference_index_payload
...
from .physical_validation_report import (build_physical_validation_report,
...
from .validation import (validate_ecology, validate_physical_world, validate_regions,
...
def _commit(repository: WorldArtifactRepository,
            stage: PhysicalStageCommit) -> WorldArtifact[Any]:
    artifact = WorldArtifact.build(
        stage.kind, stage.payload,
...
def _publish_grid_catalog(
    repository: DenseGridRepository, grid: GridSpec,
    layers: tuple[tuple[str, IntGrid[int]], ...], worker_count: int,
) -> DenseGridCatalog:
    ...
    return DenseGridCatalog(
        "storyteller.dense-grid-catalog.v1", grid,
...
class PhysicalWorldResult(Mapping[str, Union[int, str]]):
    ...
    def __getitem__(self, key: str) -> int | str:
        values: dict[str, int | str] = {
            "world_index": self.world_index, "artifacts": self.artifacts,
            "regions": self.regions, "routes": self.routes, "maps": self.maps,
        }
        try:
            return values[key]
        except KeyError:
...
def physical_stage_fingerprint(
    spec: WorldSpec, kind: str, registry_hashes: Mapping[str, str] | None = None,
) -> ProducerFingerprint:
    ...
    return ProducerFingerprint(hashlib.sha256(canonical_json({
        "algorithm": "physical-world-v1", "spec": spec, "registries": selected,
    })).hexdigest())


def generate_physical_world(
    spec: WorldSpec, seed: int, output: str | Path, *, worker_count: int = 1,
) -> PhysicalWorldResult:
    ...
    def commit(kind: str, payload: object,
               dependencies: tuple[WorldArtifact[Any], ...] = ()) -> WorldArtifact[Any]:
        ...
        return _commit(repository, PhysicalStageCommit(
            kind, payload, dependencies, physical_stage_fingerprint(spec, kind, registry_hashes),
        ))
    ...
    terrain_payload = {
        ...
        for item in fields(terrain) if item.name not in terrain_grid_fields
    }
    ...
    geology_payload = {"algorithm_version": geology.algorithm_version,
    ...
    geology_catalog = _publish_grid_catalog(
        chunk_repository, grid, geology_layers, worker_count,
    )
    ...
    hydrology_payload = {
        ...
        for item in fields(hydrology) if item.name not in hydrology_grid_fields
    }
    ...
    hydrology_catalog = _publish_grid_catalog(
        chunk_repository, grid, hydrology_layers, worker_count,
    )
    hydrology_catalog_ref = commit(
        "hydrology_grid_catalog", hydrology_catalog, (hydrology_ref,),
    )
    ...
    climate_payload = {"algorithm_version": climate.algorithm_version,
                       "season_count": len(climate.seasons),
                       "water_ledger": climate.water_ledger}
    climate_ref = commit("climate", climate_payload,
                         (terrain_ref, hydrology_ref, hydrology_catalog_ref))
    ...
    climate_catalog = _publish_grid_catalog(
        chunk_repository, grid, tuple(climate_layers), worker_count,
    )
    ...
    soil_ref = commit(
        "soil", {"algorithm_version": soil.algorithm_version},
        (terrain_ref, geology_ref, geology_catalog_ref, hydrology_ref,
         hydrology_catalog_ref, climate_ref, climate_catalog_ref),
    )
    ...
    soil_catalog = _publish_grid_catalog(
        chunk_repository, grid, soil_layers, worker_count,
    )
    ...
    biome_ref = commit("biomes", {"algorithm_version": biomes.algorithm_version},
    ...
    biome_catalog = _publish_grid_catalog(
        chunk_repository, grid, biome_layers, worker_count,
    )
    ...
    resource_ref = commit("resources", {
        "algorithm_version": resources.algorithm_version,
        "deposits": resources.deposits,
    },
                          (geology_ref, geology_catalog_ref, soil_ref, soil_catalog_ref,
    ...
    resource_catalog = _publish_grid_catalog(
        chunk_repository, grid, resource_layers, worker_count,
    )
    ...
    species_ref = commit("species", {"algorithm_version": ecology.algorithm_version,
                                     "species": ecology.species},
                         (biome_ref, biome_catalog_ref))
    ecology_ref = commit("ecology", ecology, (biome_ref, biome_catalog_ref, species_ref))
    region_ref = commit("regions", {
        "algorithm_version": regions.algorithm_version,
        "cost_model": REGION_COST_MODEL,
        "regions": regions.regions,
    },
                        (hydrology_ref, hydrology_catalog_ref, climate_ref, climate_catalog_ref,
                         biome_ref, biome_catalog_ref))
    region_catalog = _publish_grid_catalog(
        chunk_repository, grid, (("region_cell_region", regions.cell_region),), worker_count,
    )
    ...
    route_ref = commit("routes", routes,
                       (region_ref, region_catalog_ref, resource_ref, resource_catalog_ref,
                        climate_ref, climate_catalog_ref))
    spatial_index = build_spatial_index(regions, routes, grid)
    spatial_payload = spatial_index_payload(
        spatial_index, region_catalog_ref.artifact_id, region_ref.artifact_id, route_ref.artifact_id,
    )
    spatial_ref = commit("spatial_index", spatial_payload,
                         (region_catalog_ref, region_ref, route_ref))
    ...
    reference_index = ReferenceIndex.build(
        terrain, hydrology, regions, routes, resources, ecology,
    )
    reference_sources = {
        "ecology": ecology_ref.artifact_id, "hydrology": hydrology_ref.artifact_id,
        "regions": region_ref.artifact_id, "resources": resource_ref.artifact_id,
        "routes": route_ref.artifact_id, "species": species_ref.artifact_id,
    }
    ...
    validate_reference_index_payload(reference_ref.payload, reference_payload,
                                     reference_ref.depends_on)
    ...
    scalar_sources = {
        ...
        "climate": ("climate_grid_catalog", climate_catalog_ref.artifact_id,
                    "climate_annual_temperature_millic"),
        "hazard": ("climate_grid_catalog", climate_catalog_ref.artifact_id,
                   "climate_season_00_hazard_ppm"),
        "hydrology": ("hydrology_grid_catalog", hydrology_catalog_ref.artifact_id,
                       "hydrology_accumulation"),
        "political": ("region_grid_catalog", region_catalog_ref.artifact_id,
                      "region_cell_region"),
        "resource": ("resource_grid_catalog", resource_catalog_ref.artifact_id,
    ...
    map_layers = build_map_layers(grid, scalar_sources, region_ref.artifact_id,
                                  route_ref.artifact_id, regions, routes)
    map_layers_ref = commit(
        "map_layers", map_layers,
        (biome_catalog_ref, climate_catalog_ref, hydrology_catalog_ref,
         region_catalog_ref, region_ref, resource_catalog_ref, route_ref,
         soil_catalog_ref, catalog_ref),
    )
    scalar_values = {
    ...
    map_paths = render_maps(root / "maps", terrain, biomes, regions, routes,
                            map_layers, scalar_values)
    map_payload = build_map_manifest(root, map_paths, map_layers, regions)
    validate_map_manifest(root, map_payload, map_layers)
    map_ref = commit(
        "maps", map_payload,
        (biome_catalog_ref, biome_ref, climate_catalog_ref, hydrology_catalog_ref,
         map_layers_ref, region_catalog_ref, region_ref, resource_catalog_ref,
         route_ref, soil_catalog_ref, catalog_ref),
    )
    refs = (plates_ref, terrain_ref, catalog_ref, geology_ref, geology_catalog_ref, hydrology_ref,
            hydrology_catalog_ref, climate_ref, climate_catalog_ref, soil_ref, soil_catalog_ref, biome_ref,
            biome_catalog_ref,
            resource_ref, resource_catalog_ref, species_ref, ecology_ref, region_ref,
            region_catalog_ref, route_ref, spatial_ref, reference_ref, map_layers_ref, map_ref)
    ...
    return PhysicalWorldResult(index_ref.artifact_id, len(refs) + 2,
                               len(regions.regions), len(routes.routes), len(map_paths))
