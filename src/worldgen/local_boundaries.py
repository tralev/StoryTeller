...
CARDINAL_EDGES = (("north", 0, -1), ("east", 1, 0), ("south", 0, 1), ("west", -1, 0))
...
def local_boundary_from_mapping(value: Mapping[str, object]) -> LocalBoundaryConditions:
    ...
    for edge in edges_raw:
        ...
        edges.append(
            MacroBoundaryEdge(
                direction,
                edge_optional_integer("neighbor_cell"),
                edge_integer("elevation_mm"),
                edge_optional_integer("neighbor_elevation_mm"),
                edge_boolean("coastline"),
                edge_optional_boolean("neighbor_coastline"),
                edge_strings("river_edge_ids"),
                edge_strings("route_ids"),
                edge_integer("annual_temperature_millic"),
                edge_integer("annual_precipitation_mm"),
                edge_strings("deposit_ids"),
                edge_integer("renewable_yield"),
                region_id,
                owner_id,
            )
        )
    ...
    return LocalBoundaryConditions(
        ...
        str(value["region_id"]),
        integer("macro_cell"),
        integer("elevation_mm"),
        integer("rock_class_id"),
        integer("strata_id"),
        integer("parent_material_id"),
        boolean("fault"),
        boolean("volcanic"),
        boolean("coastline"),
        strings("river_edge_ids"),
        strings("lake_ids"),
        integer("aquifer_capacity_mm"),
        integer("annual_temperature_millic"),
        integer("annual_precipitation_mm"),
        integer("weather_regime"),
        strings("deposit_ids"),
        integer("renewable_yield"),
        strings("route_ids"),
        str(value["civilization_id"]),
        str(value["culture"]),
        str(value["settlement_id"]),
        str(value["settlement_status"]),
        integer("settlement_population"),
        strings("building_ids"),
    ...
    for direction, dx, dy in CARDINAL_EDGES:
        ...
        river_ids = tuple(
            sorted(
                ...
                if {edge.upstream, edge.downstream} == pair
            )
        )
        route_ids = tuple(
            sorted(
        ...
        deposits = tuple(
            sorted(item.deposit_id for item in resources.deposits if boundary_cell in item.cells)
        )
        ...
        result.append(
            MacroBoundaryEdge(
                ...
                river_ids,
                route_ids,
                int(climate.annual_temperature_millic.values[boundary_cell]),
                int(climate.annual_precipitation_mm.values[boundary_cell]),
                deposits,
                int(resources.renewable_yield.values[boundary_cell]),
                region_id,
...
def _derive_local_boundaries(world: WorldView) -> tuple[LocalBoundaryConditions, ...]:
    ...
    for site in sorted(world.sites(), key=lambda item: item.fact_id):
        ...
        if region is None or cell not in region.value["cells"] or settlement is None:
            raise ValueError(
                f"WG-LOCAL-BOUNDARY: invalid site-region-settlement join {site.fact_id}"
            )
        ...
        river_ids = tuple(
            sorted(
                f"river:{edge.upstream}:{edge.downstream}"
                for edge in hydrology.rivers
                if cell in {edge.upstream, edge.downstream}
            )
        )
        lake_ids = tuple(sorted(lake.lake_id for lake in hydrology.lakes if cell in lake.cells))
        deposit_ids = tuple(
            sorted(deposit.deposit_id for deposit in resources.deposits if cell in deposit.cells)
        )
        route_ids = tuple(
            sorted(
                route.fact_id
        ...
        workshops = tuple(
            sorted(str(item["workshop_id"]) for item in settlement.value["workshops"])
        )
        result.append(
            LocalBoundaryConditions(
                LOCAL_BOUNDARY_ALGORITHM_VERSION,
                stable_id(
                    "local_boundary",
                    int(world.payload("world_index")["seed"]),
                ...
                cell,
                int(terrain.values[cell]),
                int(geology.rock_class_id.values[cell]),
                int(geology.strata_id.values[cell]),
                int(geology.parent_material_id.values[cell]),
                bool(geology.fault.values[cell]),
                bool(geology.volcano.values[cell]),
                bool(hydrology.coastline.values[cell]),
                river_ids,
                lake_ids,
                int(hydrology.aquifer_capacity_mm.values[cell]),
                int(climate.annual_temperature_millic.values[cell]),
                int(climate.annual_precipitation_mm.values[cell]),
                int(climate.weather_regime.values[cell]),
                deposit_ids,
                int(resources.renewable_yield.values[cell]),
                route_ids,
                civilization_id,
                str(civilization.value["culture"]),
                str(settlement.value["settlement_id"]),
                str(settlement.value["status"]),
                int(settlement.value["population"]),
                tuple(sorted(str(item) for item in settlement.value["buildings"])),
                workshops,
                inventory,
                _edge_constraints(world, cell, region_by_cell, owner_by_region),
                source_ids,
            )
        )
    return tuple(result)
...
def validate_local_boundaries(
    world: WorldView,
    boundaries: tuple[LocalBoundaryConditions, ...],
) -> None:
    ...
    expected_sources = tuple(world.artifact_ids[kind] for kind in LOCAL_BOUNDARY_SOURCE_KINDS)
    if any(
