...
def _resolve_symbol(symbol: str) -> bool:
    ...
    for split in range(len(parts), 0, -1):
        ...
        try:
            ...
            if not error.name or not (module_name == error.name or module_name.startswith(error.name + ".")):
                raise
            continue
...
def validate_evidence(root: str | Path | None = None,
                      requirements: tuple[Requirement, ...] | None = None) -> list[str]:
    ...
    for requirement in selected:
        ...
        candidates = ([project / test_path] if "/" in test_path
                      else list((project / "tests").rglob(
