...
The recovered clauses are embedded here as executable evidence. Validation
fails if IDs collide or a mapping points outside the requirement catalog.
"""
...
def _clauses(prefix: str, requirement: str, anchors: tuple[str, ...]) -> tuple[SourceClause, ...]:
    return tuple(
        SourceClause(f"AUDIT-{prefix}-{index:02d}", anchor, requirement)
...
SOURCE_CLAUSES = (
    *_clauses(
    ...
    SourceClause(
        "AUDIT-COSMO-05",
        "Magic transformation must go through explicit events paying costs",
        "WG-SOC-003",
    ),
    SourceClause(
        "AUDIT-COSMO-06",
        "Every belief has `epistemic_status` (true/false/uncertain/metaphorical)",
        "WG-SOC-003",
    ),
    SourceClause(
        "AUDIT-LANG-01",
        "`Language` dataclass with phoneme inventory, syllable patterns, morphemes",
        "WG-SOC-002",
    ),
    ...
    *_clauses(
        ...
        "WG-SOC-013",
        (
            "Deterministic palette with contrast constraints",
            "Background division and overlay motif",
    ...
    *_clauses(
        ...
        (
            ...
            "Migration corridors as spatial artifacts",
    ...
    *_clauses(
        ...
        "WG-ROUTE-007",
        (
            "Frozen colour tables for all layer types",
            "Label placement algorithm",
            "Political, travel, hazard maps",
            "Derived presentation maps never replace authoritative facts",
        ),
    ),
    *_clauses(
        "OPPORTUNITY",
    ...
    *_clauses(
        ...
        "WG-LOCAL-003",
        (
            "Building interiors as separate local-map features",
            "Items as local entities with ownership/stats",
...
def validate_source_coverage() -> list[str]:
    """Validate the self-contained recovered-clause mapping."""
    errors: list[str] = []
    ...
    if len(anchors) != len(set(anchors)):
        errors.append("duplicate source clause anchor")
    for clause in SOURCE_CLAUSES:
        ...
        if not clause.anchor.strip():
            errors.append(f"empty source clause anchor: {clause.clause_id}")
    return errors
