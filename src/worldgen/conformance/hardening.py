...
HARDENING_MATRIX = (
    HardeningCase("property", "fast", "tests/worldgen/test_properties.py",
    ...
    HardeningCase(
        "security", "fast", "tests/v2/test_security.py::test_path_traversal_is_rejected",
...
def cases_for_tier(tier: HardeningTier) -> tuple[HardeningCase, ...]:
    ...
    return tuple(
        case for case in HARDENING_MATRIX
        if tier == "release" or case.tier == "fast"
    )
