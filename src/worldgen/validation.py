...
from .physical_models import (BiomeLayer, ClimateLayer, DrainageTerminalKind,
                              EcologyLayer, Hydrology, PlateBoundaryClass, RegionLayer,
                              ResourceLayer, RouteLayer, SoilLayer, Terrain)
...
from .physical_terrain import (LAND_FRACTION_TOLERANCE_PPM, MAX_ELEVATION_MM,
                               MIN_ELEVATION_MM)
...
def validate_terrain_contract(terrain: Terrain, spec: WorldSpec) -> None:
    ...
    actual_land_ppm = div_round_half_up(sum(terrain.land.values) * 1_000_000,
                                        terrain.grid.cell_count)
...
def validate_regions(terrain: Terrain, regions: RegionLayer) -> None:
    ...
    for number, region in enumerate(regions.regions, 1):
        ...
        expected_boundary = tuple(cell for cell in region.cells
                                  if any(owner[neighbor] != number
                                         for neighbor in terrain.grid.neighbors4(cell)))
        mean_x = div_round_half_up(sum(terrain.grid.coordinate(cell).x
                                      for cell in region.cells), len(region.cells))
        mean_y = div_round_half_up(sum(terrain.grid.coordinate(cell).y
                                      for cell in region.cells), len(region.cells))
        expected_center = min(region.cells, key=lambda cell: (
            abs(terrain.grid.coordinate(cell).x - mean_x)
            + abs(terrain.grid.coordinate(cell).y - mean_y), cell,
        ))
        ...
        if any(region.region_id not in neighbor_map.get(neighbor, set())
...
def validate_ecology(ecology: EcologyLayer, regions: RegionLayer) -> None:
    ...
    if any(not 0 <= item.habitat_suitability_ppm <= 1_000_000
           or item.carrying_capacity < 0 or item.population < 0
           or item.extinct != (item.population == 0) for item in ecology.regional_populations):
        raise WorldInvariantError("WG-ECOLOGY: invalid regional population")
    if any(entry.year < 1 or entry.species_id not in species_ids or entry.region_id not in region_ids
           or min(entry.population_before, entry.births, entry.deaths, entry.immigrants,
                  entry.emigrants, entry.population_after) < 0
           or entry.population_before + entry.births - entry.deaths + entry.immigrants
              - entry.emigrants != entry.population_after for entry in ecology.transition_ledger):
...
def validate_physical_world(terrain: Terrain, hydrology: Hydrology, climate: ClimateLayer,
                            soil: SoilLayer, biomes: BiomeLayer, resources: ResourceLayer,
    ...
    grids = (terrain.elevation_mm, terrain.land, terrain.continent_id,
             ...
             soil.depth_mm, soil.fertility_ppm, soil.drainage_ppm,
             soil.erosion_class, biomes.biome_id, resources.geology_id, regions.cell_region)
    ...
    for index in terrain.grid.indices():
        values = (soil.depth_mm.values[index], soil.fertility_ppm.values[index],
                  soil.drainage_ppm.values[index], soil.erosion_class.values[index])
        if not terrain.land.values[index]:
            if values != (0, 0, 0, 0):
                raise WorldInvariantError("WG-SOIL: ocean soil must be empty")
        elif (not 100 <= values[0] <= 5_000 or not 20_000 <= values[1] <= 1_000_000
              or not 0 <= values[2] <= 1_000_000 or values[3] not in (1, 2, 3)):
    ...
    for season_index, (season, ledger) in enumerate(zip(climate.seasons, climate.water_ledger)):
        seasonal_grids = (
            season.temperature_millic, season.precipitation_mm, season.evaporation_mm,
            season.snowpack_mm, season.ice, season.storm_ppm, season.wind_x_mmps,
            season.wind_y_mmps, season.hazard_ppm,
        )
        ...
        for index in range(count):
            ...
            if (not 0 <= evaporation <= rain or not 0 <= snow <= rain - evaporation
                    or ice not in (0, 1) or not 0 <= storm <= 1_000_000):
    ...
    for pass_index, entry in enumerate(terrain.erosion_ledger):
        if (entry.pass_index != pass_index or entry.mass_before_mm != entry.mass_after_mm
                or entry.thermal_moved_mm < 0 or entry.hydraulic_moved_mm < 0
        ...
        if pass_index and terrain.erosion_ledger[pass_index - 1].mass_after_mm != entry.mass_before_mm:
            raise WorldInvariantError("WG-EROSION: discontinuous mass ledger")
    if (terrain.erosion_ledger
            and terrain.erosion_ledger[-1].mass_after_mm != sum(terrain.elevation_mm.values)):
        raise WorldInvariantError("WG-EROSION: final mass ledger mismatch")
    centers = tuple(plate.center for plate in terrain.plates)
    if len(centers) != len(set(centers)) or any(center not in terrain.grid.indices() for center in centers):
        raise WorldInvariantError("WG-PLATES: plate centres must be unique and in bounds")
    for index in terrain.grid.indices():
        point = terrain.grid.coordinate(index)
        expected_owner = min(range(len(terrain.plates)), key=lambda plate_index: (
        ...
        neighbors = sorted({terrain.plate_id.values[cell] for cell in terrain.grid.neighbors4(index)
        ...
        if neighbors:
            expected_boundary = classify_plate_boundary(
                terrain.grid, terrain.plates[owner - 1], terrain.plates[neighbors[0] - 1],
            )
    ...
    for index, target in enumerate(hydrology.flow_to.values):
        ...
        if hydrology.filled_elevation_mm.values[target] > hydrology.filled_elevation_mm.values[index]:
    ...
    terminal_ids = {
        outlet: number for number, outlet in enumerate(
            sorted({terminal[index] for index in terrain.grid.indices()
                    if terrain.land.values[index]}), 1,
        )
    }
    if any(hydrology.watershed_id.values[index] != (
    ...
    for terminal_record, (cell, watershed_id) in zip(hydrology.terminals, expected_terminals):
        expected_kind = (DrainageTerminalKind.OCEAN if not terrain.land.values[cell]
                         else DrainageTerminalKind.CLOSED_BASIN)
        if (terminal_record.terminal_id != f"terminal_{watershed_id:04d}"
                or terminal_record.cell != cell or terminal_record.kind != expected_kind
    ...
    for number, lake in enumerate(hydrology.lakes, 1):
        if (lake.lake_id != f"lake_{number:04d}" or not lake.cells
    ...
    for index in terrain.grid.indices():
        ...
        expected_delta = int(
            bool(expected_coast) and hydrology.flow_to.values[index] >= 0
    ...
    for deposit in resources.deposits:
        if (deposit.deposit_id in deposit_ids or deposit.resource not in RESOURCE_DENSITY_KG_M2
                or len(deposit.cells) < 2 or tuple(sorted(set(deposit.cells))) != deposit.cells
        ...
        if (reached != cells or rock != {deposit.rock_class_id}
                or strata != {deposit.strata_id} or fault != deposit.fault_related
                or volcano != deposit.volcanic_related):
            raise WorldInvariantError("WG-RESOURCE-DEPOSIT: geometry or provenance mismatch")
        expected_resource = ("gems" if volcano else
                             ("copper" if deposit.rock_class_id % 2 == 0 else "tin") if fault else
                             {1: "coal", 2: "iron", 3: "flux_stone", 4: "copper", 5: "iron"}
                             [deposit.rock_class_id])
        expected_quantity = div_round_half_up(
            len(cells) * terrain.grid.metres_per_world_cell ** 2
            * RESOURCE_DENSITY_KG_M2[deposit.resource] * deposit.grade_ppm,
            1_000_000,
        )
    ...
    for route in routes.routes:
        ...
        if (route.end_region not in neighbor_map[route.start_region]
        ...
        if (rule is None or route.cost_unit != COST_UNIT
                or route.source_ids != expected_sources or route.annual_maintenance < 0):
        ...
        expected_maintenance = div_round_half_up(
            route.distance_m * maintenance_rate, 1_000,
        )
        ...
        for season_index, path in enumerate(route.seasonal_cells):
            if (not path or path[0] != route.cells[0] or path[-1] != route.cells[-1]
                    ...
                    or any(bool(terrain.land.values[cell]) != (rule["surface"] == "land")
                           for cell in path)
                    or any(target not in terrain.grid.neighbors4(source)
