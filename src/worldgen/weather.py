...
from .physical_models import ClimateLayer, ClimateWaterLedger, Hydrology, SeasonProfile, Terrain
...
def directional_moisture_pass(
    grid: GridSpec,
    elevation_mm: tuple[int, ...],
    land: tuple[int, ...],
    moisture_mm: tuple[int, ...],
    wind_x_mmps: tuple[int, ...],
    ...
    for index in grid.indices():
        ...
        upstream_x = point.x + (
            -1 if wind_x_mmps[index] > 0 else 1 if wind_x_mmps[index] < 0 else 0
        )
        upstream_y = point.y + (
            -1 if wind_y_mmps[index] > 0 else 1 if wind_y_mmps[index] < 0 else 0
        )
...
def generate_weather(
    terrain: Terrain, hydrology: Hydrology, *, axial_tilt_millidegrees: int, relaxation_passes: int
) -> ClimateLayer:
    grid = terrain.grid
    ...
    for season in range(4):
        ...
        for index in grid.indices():
            ...
            temperatures.append(
                solar_temperature_millic(
                    signed_latitude_ppm,
                    terrain.elevation_mm.values[index],
        ...
        for index in grid.indices():
            ...
            evaporated = min(
                rain,
                div_round_half_up(
                    max(0, temperatures[index] + 10_000),
                    100,
                ),
            )
            ...
            storm = min(1_000_000, rain * 180 + abs(wind_x[index]) * 60 + abs(wind_y[index]) * 80)
            storms.append(storm)
            hazards.append(
                min(
                    1_000_000,
                    storm + rain * 40 + (300_000 if hydrology.glacier.values[index] else 0),
                )
            )
        seasons.append(
            SeasonProfile(
                IntGrid(grid, tuple(temperatures)),
                IntGrid(grid, tuple(precipitation)),
                IntGrid(grid, tuple(evaporation)),
                IntGrid(grid, tuple(snowpack)),
                IntGrid(grid, tuple(ice)),
                IntGrid(grid, tuple(storms)),
                IntGrid(grid, tuple(wind_x)),
                IntGrid(grid, tuple(wind_y)),
    ...
    regime = tuple(
        ...
        if not terrain.land.values[i]
            ...
            if annual_rain[i] < 1_000
            else 3
            if annual_rain[i] < 4_000
            else 4
        )
        for i in grid.indices()
    )
    return ClimateLayer(
        ...
        tuple(seasons),
        tuple(water_ledger),
        IntGrid(grid, annual_temp),
        IntGrid(grid, annual_rain),
        IntGrid(grid, regime),
    )
