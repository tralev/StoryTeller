...
from ..worldgen.numeric import identity as id_component
...
def build_knowledge_index(
    ...
    def add(
        ...
        entries.append(
            KnowledgeEntry(
                entry_id,
                kind,
                normalize_knowledge(text),
                tuple(sorted(set(sources))),
                (),
                tuple(sorted(set(outgoing))),
                reveal,
            )
        )
    ...
    for claim in bible.local_entities:
        add(
            ...
            claim.entity_id,
            asdict(claim),
            claim.authoritative_refs,
            (claim.contained_by,),
            (reveal_by_ref.get(claim.contained_by, node_ids[0]),),
        )
    ...
    for node in graph.nodes:
        add(
            ...
            node.node_id,
            asdict(node),
            node.authoritative_refs,
            tuple(choice.target_node for choice in node.choices),
            (node.node_id,),
        )
    ...
    for local in local_maps:
        sources = tuple(
            sorted({source for feature in local.features for source in feature.source_ids})
        )
        add(
            "local_map",
            local.site_id,
