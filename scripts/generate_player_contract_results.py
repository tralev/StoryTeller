...
def main()->None:
 ...
 catalog=json.loads((ROOT/"tests/fixtures/v2/catalog.json").read_text());out={}
 for item in catalog["scenarios"]:
  result=validate_v2_package(ROOT/"tests/fixtures/v2"/item["path"])
  out[item["id"]]={"outcome":"accepted" if result.accepted else "invalid",
                   "issue_codes":[issue.code for issue in result.issues]}
 value={"format":"storyteller.contract-results.v2","scenarios":out};path=Path(args.output)
