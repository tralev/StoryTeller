...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ok = check(
            ...
            json.loads(bible_path.read_text())
            == {"world_name": "Immediate Test", "entities": {"characters": []}},
        )
        passed += ok
        failed += not ok
        ...
        ok = check(
            f"All {len(all_files)} JSON files on disk",
            len(existing) == len(all_files),
        ...
        ok = check(
            "Overwrite: bible.json reflects new content",
        ...
        ok = check(
            "After del: style_bible.json GONE from disk", not (out / "style_bible.json").exists()
        )
        passed += ok
        failed += not ok
        ...
        ok = check(
            "New context reads bible from disk",
            ctx2.outputs.get("bible")
            == {"world_name": "Updated World", "entities": {"characters": ["new"]}},
        )
        passed += ok
        failed += not ok

        ok = check(
            "New context reads story from disk",
        ...
        if failed == 0:
            print("  \u2714 ArtifactStore write-through works correctly!")
            print("  Every context.outputs[key] = data flushes to disk immediately.")
        else:
